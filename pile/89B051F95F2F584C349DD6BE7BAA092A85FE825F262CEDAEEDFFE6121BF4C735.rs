...
fn should_be_bitransmutable() {
    ...
    assert::is_transmutable::<  u32,  u128>();
...
fn should_reject_extension() {
    ...
    assert::incomplete_features::<   i8,   u32>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  f32,   u64>(); // check-fail
