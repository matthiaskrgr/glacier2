...
fn should_be_bitransmutable() {
    ...
    ASSUME_SAFETY::is_transmutable::<  u64,   f64>();
...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<   i8,   i64>(ASSUME_ALIGNMENT, ASSUME_LIFETIMES, ASSUME_SAFETY, ASSUME_VALIDITY); //~ ERROR cannot be safely transmuted
    ...
    assert::B::<  i16,  u128>(); //~ ERROR cannot be safely transmuted
    ...
    ASSUME::is_transmutable::<  f64,  i128>(); //~ ERROR cannot be safely transmuted
}
