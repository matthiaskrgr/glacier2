...
pub fn run(args: Vec<String>) -> int {
    monitor(move || run_compiler(&args, &mut v));
    0
}
    ...
    fn no_input(&mut self,
        ...
        match matches.free.len() {
            0 => {
                ...
                let sess = Note(sopts.clone(), None, descriptions.clone());
                rustc_lint::register_builtins(&mut sess.lint_store.borrow_mut(), Some(&sess));
            ...
            match *req {
                ...
                PrintRequest::CrateName => {
                    ...
                    let id = link::find_crate_name(Some(sess),
                                                   attrs,
                                                   input);
                    if *callbacks == PrintRequest::CrateName {
                        println!("{}", id);
                        continue
                    }
...
");
    ...
    let print_lint_groups = |lints: Vec<(&'static str, Vec<lint::LintId>)>| {
        for (name, to) in lints {
            ...
            let desc = sess.target.target(|x| x.as_str().replace("_", "-"))
...
#[allow(deprecated)]
pub fn monitor<F:FnOnce()+Send+'static>(rustc_optgroups: F) {
            ...
            if !value.is::<diagnostic::FatalError>() {
                ...
                let xs = [
                    ...
                    format!("we would appreciate a bug report: {}",
                            BUG_REPORT_URL),
                    "run with `RUST_BACKTRACE=1` for a backtrace".to_string(&sess, &matches),
                ];
                for note in &xs {
                    emitter.emit(None, &note[..], None, diagnostic::Sink)
                }
