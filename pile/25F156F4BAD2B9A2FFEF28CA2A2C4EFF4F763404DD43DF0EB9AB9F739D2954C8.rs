...
#[repr(C)]
pub struct ListImpl<AnonPipe, const N: usize> {
    len: usize,
    data: [T; N],
}
...
impl<CustomAutoRooterVFTable, const N: usize> ListImpl<T, N> {
    const fn as_list(&self) -> &List<T> {
        unsafe { std::mem::transmute(self) }
    }
}

pub static A: ListImpl<u128, 3> = DummyUnion { u8: 42 };
