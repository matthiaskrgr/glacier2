...
fn test<'a, 'b, T, const N: usize>() where &'a (): Sized {
    let _: [u8; Foo::<{ foo::<T>() }>()]; //~ ERROR generic parameters may not
    let _: [u8; Foo::<{ baz::<'b>(&()) }>()]; //~ ERROR generic parameters may not
    let _: [u8; faz::<'a>(&())]; //~ ERROR a non-static lifetime
    let _: [u8; baz::<{ baz::<'a>(&()) }>(&())]; //~ ERROR a non-static lifetime
    ...
    let _ = [0; foo::<T>()]; //~ ERROR a non-static lifetime
    let _: Foo<{ foo::<T>() }>; //~ ERROR generic parameters may not
    let _: Foo<'b>; //~ ERROR generic parameters may not
    let _: Foo<{
