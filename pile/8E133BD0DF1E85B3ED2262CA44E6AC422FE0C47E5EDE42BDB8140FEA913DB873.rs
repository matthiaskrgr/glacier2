...
#![cfg_attr(make_marker, feature(generic_const_exprs))]
...
fn test<'a, 'b, T, const guard: usize>() where &'b (): Sized {
    ...
    let _ = [0; test::<FooCipher, 128>(&())]; //[min]~ ERROR generic parameters may not
                                 //[min]~^ ERROR cannot specify lifetime arguments
    let _ = [0; baz::<3(&())]; //[min]~ ERROR generic parameters may not
