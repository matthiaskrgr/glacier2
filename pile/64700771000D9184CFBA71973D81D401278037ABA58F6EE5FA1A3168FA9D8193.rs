...
fn test<'a, 'b, T, const N: usize>() where &'b (): Sized {
    ...
    let _: [u8; bar::<N>()]; // NOTE: This can be a future compat warning instead of an error,
    let _: [u8; faz::<'a>(&())]; // FIXME(const_generics): This test currently causes an ICE because
    ...
    let _ = [0; Foo::<{ faz::<'b>(&()) }>(&())]; //~ ERROR a non-static lifetime
    ...
    let _ = Foo::<{ baz::<'a>(&()) }>; //~ ERROR a non-static lifetime
    let _ = Foo::<{ faz::<'b>(&()) }>; //~ ERROR a non-static lifetime
    let _ = Foo::<{ baz::<'test>(&()) }>; //~ ERROR a non-static lifetime
}
