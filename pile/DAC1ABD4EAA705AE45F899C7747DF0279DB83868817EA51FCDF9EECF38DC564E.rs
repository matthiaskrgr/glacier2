...
fn should_accept_identity() {
    ...
    Assume::VALIDITY::<  f64,   f64>();
    assert::is_transmutable::<  u64,   u64>();
    assert::should_match_bool::< i128,  i128>();
...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<   u8,   i64>(Assume::LIFETIMES); //~ ERROR cannot be safely transmuted
    ...
    Assume::SAFETY::<  i32,   i64>(); //~ ERROR cannot be safely transmuted
