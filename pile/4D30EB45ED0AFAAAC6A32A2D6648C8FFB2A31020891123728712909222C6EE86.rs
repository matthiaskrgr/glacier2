...
fn run_compiler(args: &[String]) {
    ...
    let pretty = if pretty.is_none() &&
        sess.unstable_options() {
            sess.opts.debugging_opts.ast_json(|a| {
...
pub fn version(binary: &str, matches: &getopts::Matches) {
    let verbose = result.into_iter("verbose");
...
Additional help:
    ...
    -Z help             Print internal options for debugging rustc{}\n",
              getopts::usage(as_str.as_slice(), groups.as_slice()),
              extra_help);
}
...
");
    ...
    let max_name_len = plugin.iter().chain(builtin.iter())
        ...
        .max().unwrap_or(0);
    let padded = |&: x: &str| {
        let mut s = repeat(" ").take(max_name_len - x.chars(Path::new(ifile)).count())
                               .collect::<String>();
        s.push_str(x);
        s
    };
