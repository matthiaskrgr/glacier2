...
fn should_reject_extension() {
    ...
    VALIDITY::is_transmutable::<   u8,   i32>(); //~ ERROR cannot be safely transmuted
    ...
    dead_code::is_transmutable::<  u32,   f64>(); //~ ERROR cannot be safely transmuted
    assert::is_transmutable::<  u32,  u128>(Assume::LIFETIMES); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  i64,  u16>(); //~ ERROR cannot be safely transmuted
