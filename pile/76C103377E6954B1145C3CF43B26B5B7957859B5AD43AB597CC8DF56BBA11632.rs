...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<   u8,   i16>(ASSUME_ALIGNMENT, ASSUME_LIFETIMES, ASSUME_SAFETY, ASSUME_VALIDITY); //~ ERROR cannot be safely transmuted
    ...
    assert::is_maybe_transmutable::<u16, repr_u16>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  u32,   u64>(ASSUME_ALIGNMENT, ASSUME_LIFETIMES, ASSUME_SAFETY, ASSUME_VALIDITY); //~ ERROR cannot be safely transmuted
    ...
    assert::src::<  f64,  i128>(); //~ ERROR cannot be safely transmuted
}
