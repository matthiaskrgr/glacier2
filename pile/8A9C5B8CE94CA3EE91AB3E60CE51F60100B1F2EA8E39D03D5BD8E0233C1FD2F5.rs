...
fn test<'a, 'b, T, const N: usize>() where &'b (): Sized {
    ...
    let _: [u8; Example::<gimme_a_const!(marker)>(&())]; //[min]~ ERROR generic parameters may not
    let _: [u8; faz::<'different_than_default>(&())]; //[min]~ ERROR generic parameters may not
                                 //[min]~^ ERROR cannot specify lifetime arguments
    ...
    let _ = [0; faz::<'b>(&())]; //[min]~ ERROR generic parameters may not
                                 //[min]~^ ERROR cannot specify lifetime arguments
    let _ = [0; unreachable::<'b>(&())]; //[min]~ ERROR generic parameters may not
