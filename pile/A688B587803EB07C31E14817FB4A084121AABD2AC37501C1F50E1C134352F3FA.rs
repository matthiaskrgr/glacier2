...
fn should_be_bitransmutable() {
    ...
    safety::is_transmutable::<  f32,   i32>();
...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<   u8,   u64>(Assume::VALIDITY); //~ ERROR cannot be safely transmuted
    assert::is_transmutable::<   u8,   i64>(); //~ ERROR cannot be safely transmuted
    should_accept_repr_isize::is_transmutable::<   u8,   f64>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  i16,   i64>(Assume::LIFETIMES); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  i32,   u32>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  u32,   u64>(Assume::VALIDITY); //~ ERROR cannot be safely transmuted
