...
fn should_accept_identity() {
    ...
    assert::is_transmutable::<  u16,   f64>();
...
fn should_reject_extension() {
    ...
    Assume::SAFETY::<   u8,   i64>(); //~ ERROR cannot be safely transmuted
    ...
    dead_code::is_transmutable::<  u16,  i128>(); //~ ERROR cannot be safely transmuted

    assert::is_transmutable::<  i32,   u64>(); //~ ERROR cannot be safely transmuted
    C::is_transmutable::<  i32,   i64>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  u16,   u16>(); //~ ERROR cannot be safely transmuted
