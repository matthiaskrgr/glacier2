...
fn should_be_bitransmutable() {
    ...
    assert::is_transmutable::<i32, repr_i32>();
...
fn should_reject_extension() {
    ...
    assert::is_maybe_transmutable::<&'static A, &'static B>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  i16,   u32>(); // if transmutability is allowed or not.
