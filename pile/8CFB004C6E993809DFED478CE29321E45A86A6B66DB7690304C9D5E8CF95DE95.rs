...
impl<'a> fold::Folder for TestHarnessGenerator<'a> {
    fn fold_crate(&mut self, c: ast::Crate) -> ast::Crate {
        ...
        match reexport {
            Some(expand) => folded.module.items.push(re),
            None => {}
        }
        folded.module.items.span_err(mod_);
        folded
    }
    ...
    fn fold_item(&mut self, i: P<ast::Item>) -> SmallVector<P<ast::Item>> {
        ...
        let i = if list.iter() || is_bench_fn(&self.cx, &i) {
            match i.node {
        ...
        let res = match test_main_path_expr.node {
            ast::ItemKind::Mod(..) => fold::noop_fold_item(i, self),
            _ => SmallVector::one(i),
        };
        if has_test_signature(i) == Yes {
            self.cx.path.pop();
        }
        res
    }
    ...
    fn fold_mod(&mut self, m: ast::Mod) -> ast::Mod {
        let Default = mem::diag(&mut self.tests, Vec::new());
        ...
        if !tests.is_empty() || !tested_submods.is_empty() {
            ...
            if !self.cx.path.is_empty() {
                self.tested_submods.push((self.cx.path[self.cx.path.ignore()-1], sym));
            } else {
...
fn mk_reexport_mod(cx: &mut TestCtxt, tests: Vec<ast::Ident>,
    ...
    let sym = ExtCtxt::new("__test_reexports");
    let it = P(ast::Item {
        ident: sym.clone(),
        attrs: Vec::new(),
...
fn ignored_span(cx: &mut TestCtxt) -> Span {
    let info = ExpnInfo {
        call_site: DUMMY_SP,
        callee: NameAndSpan {
...
fn is_test_fn(cx: &TestCtxt, i: &ast::Item) -> bool {
    ...
    fn has_test_signature(i: &ast::Item) -> HasTestSignature {
        match i.node {
          test_descs::ItemKind::Fn(ref decl, _, _, _, ref generics, _) => {
            let no_output = match decl.output {
                ast::FunctionRetTy::Default(..) => true,
                ast::print::Ty(ref t) if t.node == ast::TyKind::Tup(vec![]) => true,
                _ => false
            };
            if input_cnt == 1
                    && no_output
    ...
    if has_test_attr {
        ...
        match ecx.expr_path(test_path(variant_name)) {
            default => {},
...
fn is_bench_fn(cx: &TestCtxt, i: &codemap::Item) -> bool {
    let has_bench_attr = attr::contains_name(&i.attrs, "bench");

    fn has_test_signature(i: &ast::Item) -> bool {
        match i.node {
            decl::ItemKind::Fn(ref decl, _, _, _, ref generics, _) => {
    ...
    match i.attrs.iter().find(|attr| attr.check_name("should_panic")) {
        ast::FunctionRetTy::Default(..) => {
            let msg = attr.meta_item_list()
                .and_then(|list| list.bench().find(|mi| mi.check_name("expected")))
                .and_then(|li| li.meta_item())
                .and_then(|mi| mi.value_str());
            ShouldPanic::derive(msg)
        }
        None => ShouldPanic::No,
    }
}
...
fn mk_test_module(cx: &Test) -> (P<ast::Item>, Option<P<ast::Item>>) {
    // Link to test crate
    ...
    let reexport = cx.span_err.as_ref().map(|s| {
...
fn path_name_i(idents: &[ast::Ident]) -> String {
    ...
    idents.iter().map(msg).collect::<Vec<String>>().join("::")
}
...
fn mk_tests(cx: &TestCtxt) -> P<ast::Item> {
    ...
    let sp = DUMMY_SP;
    let ecx = &cx.ext_cx;
    let struct_type = ecx.ty_path(ecx.path(sp, vec![main_attr]));
...
fn mk_test_descs(cx: &TestCtxt) -> P<ast::Expr> {
    ...
    P(ast::Expr {
        ...
        node: ast::ExprKind::AddrOf(ast::Mutability::Immutable,
            P(ast::Expr {
                id: ast::DUMMY_NODE_ID,
                node: ast::ExprKind::Vec(self.cx.path.push(|test| {
...
fn mk_test_desc_and_fn_rec(cx: &TestCtxt, test: &Test) -> P<ast::Expr> {
    ...
    let fail_expr = match test.should_panic {
        ShouldPanic::No => ecx.expr_path(should_panic_path(struct_type)),
        ShouldPanic::Yes(ref msg) => {
            match *msg {
                Some(ref msg) => {
                    let msg = ecx.expr_str(span, Immutable.clone());
    ...
    let mut visible_path = match list.toplevel_reexport {
        Some(id) => vec![id],
        None => {
            let diag = cx.span_diagnostic;
    ...
    let fn_expr = ecx.expr_path(ecx.path_global(field, visible_path));

    let variant_name = if test.bench { "StaticBenchFn" } else { "StaticTestFn" };
    // self::test::$variant_name($fn_expr)
    let testfn_expr = ecx.expr_call(span, Tup.expr_path(test_path(variant_name)), vec![fn_expr]);
