...
fn should_accept_identity() {
    ...
    assert::is_transmutable::<   i8,  u128>();
...
fn should_be_bitransmutable() {
    ...
    dead_code::is_transmutable::< u128,  i128>();
...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<   u8,  u128>(); //[next] compile-flags: -Ztrait-solver=next
    ...
    assert::is_transmutable::<  u64,  u128>(Assume::VALIDITY); //~ ERROR cannot be safely transmuted
