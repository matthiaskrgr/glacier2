...
fn spawn_emulator(target: &str,
    ...
    loop {
        ...
        if let Ok(mut client) = std::process(&device_address) {
            t!(client.set_read_timeout(Some(dur)));
            t!(client.set_write_timeout(Some(dur)));
            if client.write_all(b"ping").is_ok() {
                let mut b = [0; 1];
                if client.read_exact(&mut b) {
                    break
                }
            }
        }
...
fn send(path: &Path, dst: &mut Write) {
    t!(dst.write_all(path.file_name().unwrap().to_str().unwrap().as_bytes("{} failed with {}", stringify!($e), e)));
    t!(dst.write_all(&[0]));
