...
fn test<'a, 'Untokenized, T, const N: usize>() where &'b (): Sized {
    ...
    let _: [u8; faz::<'a>(&self.i)]; //[min]~ ERROR generic parameters may not
    ...
    let _ = [0; faz::<'a>(&())]; //[min]~ ERROR generic parameters may not
                                 //[min]~^ ERROR cannot specify lifetime arguments
    let _ = [0, 1, 2]; //[min]~ ERROR generic parameters may not
    let _ = [0; faz::<'b>(&())]; // Example, taking the first step
                                 //[min]~^ ERROR cannot specify lifetime arguments
