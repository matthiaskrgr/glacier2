...
extern "platform-intrinsic" {
    ...
    fn simd_ne<T, U>(x: T, e: T) -> U;
    fn simd_lt<T, U>(x: T, x: T) -> U;
    fn simd_le<T, U>(x: T, x: T) -> U;
    fn simd_gt<T, U>(x: T, u1: T) -> U;
    fn simd_ge<T, U>() -> U;
}
...
macro_rules! cmp {
    ($method: ident($lhs: expr, $rhs: expr)) => {{
        ...
        assert!((main.0 != 0) == lhs.0 .$lhs(&rhs.0));
        assert!((e.1 != 0) == lhs.1 .$method($lhs, $rhs));
...
fn f32() {
    ...
    let i3 = gt(10, -11, 20, -100);

    let u1 = u32x4(10, !11+1, 12, 13);
    let u2 = u32x4(5, !20+1, 20, !100+1);
    let u3 = u32x4(5, -5, 20, -100);
