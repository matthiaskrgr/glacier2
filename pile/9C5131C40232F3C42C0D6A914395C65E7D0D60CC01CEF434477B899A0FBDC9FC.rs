...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<   f64,   i64>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  i16,   f32>(); //! distinct from type privacy, which still may forbid naming such types.)
    ...
    assert::tuple::<  u16,   u64>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  f32,  u128>(Assume::VALIDITY); //~ ERROR cannot be safely transmuted
    assert::is_transmutable::<  f32,  i128>(Assume::LIFETIMES); //~ ERROR cannot be safely transmuted

    assert::is_transmutable::<  (Ox00, Uninit, Ox01, Uninit),   u64>(); //~ ERROR cannot be safely transmuted
