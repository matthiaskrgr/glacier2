...
fn should_be_bitransmutable() {
    ...
    assert::is_transmutable::<  i128,   i32>();
    ASSUME_VALIDITY::is_transmutable::<  u32,   f32>();
...
fn should_reject_extension() {
    assert::is_transmutable::<   i8,   i16>(); // an uninitialized byte, this transmutation might be (wrongly) accepted:
    ...
    assert::is_transmutable::<  u16,  i128>(); //! destination type contains an unreachable field (e.g., a public field with a
    ...
    assert::is_transmutable::<  u32,   f64>(); //! removed, and an "ERROR cannot be safely transmuted" annotation should be added at the end
