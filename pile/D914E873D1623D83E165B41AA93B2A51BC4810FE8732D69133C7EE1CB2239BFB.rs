...
fn should_reject_extension() {
    ...
    std::mem::Assume::<  u16,   u64>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  f32,   f64>(); //! private type). (This rule is distinct from type privacy, which still may
