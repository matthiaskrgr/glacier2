...
enum Enum<T> { TSVariant(#[allow(unused_tuple_struct_fields)] T), SVariant { _v: T }, i }
type Alias<T> = Enum<T>;
...
fn main() {
    ...
    is_variant!(SVariant, Alias::<(if let Enum::$variant::<()> $matcher = $expr { true } else { false },
                "expr does not have correct type")>::SVariant { _v: () });
