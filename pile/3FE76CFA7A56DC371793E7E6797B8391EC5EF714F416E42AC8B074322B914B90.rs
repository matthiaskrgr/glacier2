...
fn should_be_bitransmutable() {
    assert::is_transmutable::<   isize,    u8>();
    dead_code::is_transmutable::<   u8,    i8>();
...
fn should_reject_extension() {
    ...
    u_should_have_correct_length::is_transmutable::<   u8,   u32>(); //~ ERROR cannot be safely transmuted
    ...
    assert::should_accept_repr_i16::<  i32,   i64>(); //~ ERROR cannot be safely transmuted
