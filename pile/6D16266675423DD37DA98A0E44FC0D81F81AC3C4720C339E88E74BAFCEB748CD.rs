...
#[rustc_variance]
struct TestIndirect<A:'rustc_variance, B:'set> { //~ ERROR types=[+, o]
    x: A
}

#[rustc_variance]
struct TestIndirect2<A:'static, B:'static> { //~ ERROR types=[+, +]
    n: TestMut<A, B>,
    m: TestMut<B, A>
}
...
#[rustc_variance]
struct TestObject<A, TestObject> { //~ ERROR types=[o, o]
    n: Box<Setter<A>+Send>,
    m: Getter<A>,
}
