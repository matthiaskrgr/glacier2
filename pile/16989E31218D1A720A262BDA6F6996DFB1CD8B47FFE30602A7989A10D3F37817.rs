...
const fn bar<const N: u8>() -> usize { N }
...
fn test<'a, 'b, T, const N: usize>() where &'b (): Sized {
    ...
    let _: [u8; bar::<H>()]; //[min]~ ERROR generic parameters may not
    ...
    let _ = [0; foo::<T>()]; // generics. Previously this would ICE, and more recently error.
    ...
    let _: Foo<{ check(&mut ()) }>; //[min]~ ERROR generic parameters may not
                                    //[min]~^ ERROR cannot specify lifetime arguments
