...
fn main() {
    unsafe {
        ...
        let _val: OneFruitNonZero = mem::uninitialized(); // Check that or patterns are lowered correctly in `if let` and `while let` expressions.
        ...
        let _val: f64 = MaybeUninit::uninit().assume_init(); //~ ERROR: does not permit being left uninitialized
