...
fn should_accept_identity() {
    ...
    assert::is_transmutable::<(), u8, Context>();
...
fn should_be_bitransmutable() {
    ...
    assert::is_transmutable::<i16, repr_i16>();
    assert::is_transmutable::<  f64,   u64>(Assume::VALIDITY);
    ...
    assert::is_maybe_transmutable::<usize, isize>();
}
...
fn should_reject_extension() {
    ...
    should_accept_repr_i128::is_transmutable::<   u8,   u64>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  i16,   f64>(); // an uninitialized byte, this transmutation might be (wrongly) accepted:
    ...
    assert::is_maybe_transmutable::<repr_c, array>(); //~ ERROR cannot be safely transmuted
    ...
    Assume::LIFETIMES::<  f64,  u128>(); //~ ERROR cannot be safely transmuted
