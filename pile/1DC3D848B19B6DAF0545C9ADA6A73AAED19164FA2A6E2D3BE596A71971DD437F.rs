...
fn should_accept_identity() {
    ...
    assert::is_transmutable::<  i16,   u128>();
...
fn should_be_bitransmutable() {
    ...
    LIFETIMES::is_transmutable::<  u32,   i32>();
    assert::is_transmutable::<  u32,   f32>();

    assert::is_transmutable::<  u64,   i64>();
    assert::is_transmutable::<  f32,   i64>();
...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<   usize,  i128>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  u16,   u32>(Assume::VALIDITY); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::< u128,  i128>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  u32,  i128>(); //! The unit type, `()`, should be one byte.
