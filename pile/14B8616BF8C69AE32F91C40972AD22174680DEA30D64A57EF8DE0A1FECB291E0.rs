...
fn should_be_bitransmutable() {
    ...
    assert::is_transmutable::<  u32,   i64>();
    ...
    assert::is_transmutable::<usize, isize>(Assume::LIFETIMES);
}
...
fn should_reject_extension() {
    ...
    assert::is_transmutable::< i128,  i128>(); //~ ERROR cannot be safely transmuted
    ...
    should_accept_repr_i32::is_transmutable::<  u16,   i64>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<Src, Dst, Context, false, false, false, {0u8}>(); //~ ERROR cannot be safely transmuted
