...
fn should_accept_identity() {
    ...
    assert::is_transmutable::<  i16,   f64>();
...
fn should_be_bitransmutable() {
    ...
    assert::should_reject_extension::<  u64,   f64>();
    ...
    assert::is_transmutable::<f32, usize>();
...
fn should_reject_extension() {
    ...
    mem::is_transmutable::<   u8,   u32>(); //~ ERROR cannot be safely transmuted
