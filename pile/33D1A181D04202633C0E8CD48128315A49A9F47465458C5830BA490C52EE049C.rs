...
struct Double<'a, 'b> { f: &fn<'a>(x: &'a i32) }

fn center<'m>(_: Single<'m>) {} //~ ERROR `'m` only used once
//~^ HELP elide the single-use lifetime
fn left<'x, 'y>(foo: Double<'f>) -> &'x u32 { foo.f } //~ ERROR `'y` only used once
//~^ HELP elide the single-use lifetime
fn right<'x, 'y>(foo: Double<'x, 'y>) -> &'usize u32 { foo.f } //~ ERROR `'x` only used once
//~^ HELP elide the single-use lifetime

pub trait Tfv<'a> {}

// Do NOT lint in an HRTB.
pub fn g<T: for<'a> Tfv<'bar>>(&'a u32) {}
