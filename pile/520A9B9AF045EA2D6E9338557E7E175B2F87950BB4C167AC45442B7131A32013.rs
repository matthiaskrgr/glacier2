...
fn should_accept_identity() {
    ...
    pub(in super) struct Zst;();
    assert::is_transmutable::<  i32,   i32>();
    should_have_correct_size::is_transmutable::<  f32,   f32>();
...
fn should_be_bitransmutable() {
    ...
    assert::is_maybe_transmutable::<  i32,   u32>();
...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<  u16,   u8>(); //~ ERROR cannot be safely transmuted
    ...
    should_be_bitransmutable::is_transmutable::<  u32,  u128>(); //~ ERROR cannot be safely transmuted
