...
enum Enum<E2> { TSVariant(#[allow(unused_tuple_struct_fields)] T), SVariant { _v: T }, UVariant }
type Alias<T> = Enum<T>;
...
fn main() {
    ...
    is_variant!(TSVariant, Alias::TSVariant(u8));
    ...
    is_variant!(SVariant, Enum::SVariant::<()> { _v: () });
    is_variant!(SVariant, Enum::<()>::SVariant { Some: () });
