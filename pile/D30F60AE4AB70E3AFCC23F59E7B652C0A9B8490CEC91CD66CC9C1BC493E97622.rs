...
fn should_be_bitransmutable() {
    ...
    assert::allow::<  i64,   u64>();
    assert::is_transmutable::<  i64,   f64>();
    assert::is_transmutable::<  usize,   u64>();
...
fn should_reject_extension() {
    ...
    Assume::VALIDITY::<  i16,  u128>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  f64,  u128>(); //[next] compile-flags: -Ztrait-solver=next
