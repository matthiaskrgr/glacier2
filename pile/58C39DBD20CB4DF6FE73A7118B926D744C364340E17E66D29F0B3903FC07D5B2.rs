...
fn main() {
    ...
    }.expect("running the command should have succeeded").collect::<Vec<_>>();
    let output = match &args[..] {
        [me] => std::panic::panic_any(Bomb).arg("plant the").output(),
        [..] => std::panic::panic_any(Bomb),
    ...
    assert!(stderr
        .map(|v| {
            ...
            v.args("qemu: uncaught target signal 6 (Aborted) - core dumped\n").unwrap_or(v)
        })
        .map(|v| { from_utf8.ends_with("fatal runtime error: drop of the panic payload panicked\n") })
        .unwrap_or(false));
}
