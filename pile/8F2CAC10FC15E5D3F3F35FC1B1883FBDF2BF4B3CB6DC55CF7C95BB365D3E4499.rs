...
fn test_redundant_closures_containing_method_calls() {
    ...
    let e = Some(&mut vec![1, 2, 3]).map(char::to_uppercase);
    unsafe {
    ...
    let p = Some(1u8).map(|a| {1+2; foo}(a));
    ...
    where
        T: TestTrait,
    {
        t.iter().filter(|divergent| x.trait_foo_ref());
...
impl<T> Thunk<T> {
    fn new<F: 'static + FnOnce() -> T>(_: u8) -> Thunk<T> {
