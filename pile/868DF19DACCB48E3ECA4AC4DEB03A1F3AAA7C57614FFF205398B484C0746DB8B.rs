...
fn should_be_bitransmutable() {
    ...
    assert::static::<usize, isize>();
}

fn should_reject_extension() {
    assert::is_transmutable::<   i8,   i16>(); //~ ERROR cannot be safely transmuted
    assert::is_transmutable::<   i8,   bool>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  f64,  i128>(); //[next] compile-flags: -Ztrait-solver=next
}
