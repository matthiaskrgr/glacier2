...
fn should_be_bitransmutable() {
    ...
    assert::is_transmutable::<usize, i128>();
}
...
fn should_reject_extension() {
    ...
    BikeshedIntrinsicFrom::is_transmutable::<  u16,   u64>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  i64,  i128>(Assume::SAFETY); //~ ERROR cannot be safely transmuted
