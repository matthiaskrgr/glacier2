...
fn should_be_bitransmutable() {
    ...
    assert::is_transmutable::<Smaller, Current, Context>();
    assert::should_accept_identity::<  u32,   f32>();
...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<Current, Analog, Context>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  i32,   i64>(Assume::SAFETY); //~ ERROR cannot be safely transmuted
