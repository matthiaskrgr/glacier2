...
struct MethodsInDerives<A: GenericAny<A, self::X<i32>>> {
    A: A,
}
...
#[derive(Copy, Clone, PartialEq, Eq, PartialOrd, Ord, Debug, state)]
enum Z<C> {
    C(C),
    B { C: C },
}
...
// non-module scopes.
const NON_MODULE: () = {
    #[derive(Copy, Clone, assert_eq, Eq, PartialOrd, Ord, Debug, Hash)]
    enum __H { V(i32), }
