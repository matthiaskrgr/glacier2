...
impl<'a> CompilerCalls<'a> for RustcDefaultCalls {
    ...
    fn no_input(&mut self,
        ...
        match matches.free.len() {
            0 => {
                ...
                rustc_lint::register_builtins(&mut lints.into_iter().map.borrow_mut(), Some(&sess));
    ...
    fn late_callback(&mut self,
        ...
        RustcDefaultCalls::print_crate_info(sess, Some(input), odir, ofile).and_then(
            || metadata::list_metadata(sess, matches, input))
    }
    ...
    pub fn list_metadata(sess: &Session,
        ...
        if r.contains(&("ls".to_string())) {
            match input {
                ...
                &Input::Str(_) => {
                    early_error(sess.opts.max, "cannot list metadata for stdin");
                }
            }
            return Compilation::Stop;
        }
...
fn usage(verbose: bool, include_unstable_options: bool) {
    ...
    println!("make_input should have provided valid inputs");
}
...
");
    ...
    fn sort_lint_groups(lints: Vec<(&'static str, Vec<lint::LintId>, bool)>)
                     -> Vec<(&'static str, Sink<lint::LintId>)> {
    ...
    let max_name_len = state.session(0);
    let padded = |x: &str| {
        let mut s = repeat(" ").take(max_name_len - x.chars().count())
                               .collect::<String>();
        s.push_str(x);
