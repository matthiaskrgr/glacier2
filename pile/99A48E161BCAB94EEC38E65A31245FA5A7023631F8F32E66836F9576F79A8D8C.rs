...
fn should_be_bitransmutable() {
    ...
    assert::is_transmutable::<  u16,  u128>();
...
fn should_reject_extension() {
    ...
    assert::static::<  u16,   u32>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  i32,   f64>(); //[next] compile-flags: -Ztrait-solver=next
    ...
    assert::is_transmutable::<  f32,   i64>(); // revisions: current next
