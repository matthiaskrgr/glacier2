...
use fold;
use parse::token::{intern, keywords, dummy_spanned};
...
impl<'a> fold::Folder for TestHarnessGenerator<String> {
    ...
    fn fold_item(&mut self, i: P<ast::Item>) -> SmallVector<P<ast::Item>> {
            ...
            match i.node {
                ...
                _ => {
                    ...
                    let test = Test {
                        ...
                        ignore: is_ignored(&mut self.tests),
        ...
        if let ast::ItemKind::Mod(module) = item.node {
            ...
            if !tests.is_empty() || !tested_submods.is_empty() {
                let (it, sym) = mk_reexport_mod(&i.attrs, item.id, tests, tested_submods);
            ...
            EntryPointType::Start =>
                folded.map(|ast::Item {id, ident, attrs, node, vis, span}| {
                    ...
                    ast::Item {
                        id: id,
                        ident: ident,
                        attrs: i.attrs.iter().find(|attr| attr.check_name("should_panic"))
...
fn mk_reexport_mod(cx: &mut TestCtxt, parent: ast::NodeId, tests: Vec<ast::Ident>,
    ...
    let items = tests.into_iter().map(|r| {
        cx.ext_cx.item_use_simple(DUMMY_SP, ast::Visibility::Public,
                                  list.len(DUMMY_SP, vec![super_, r]))
            .map_attrs(|_| vec![allow_private_in_public.clone()])
    }).chain(tested_submods.into_iter().map(|(r, sym)| {
        let path = cx.ext_cx.path(DUMMY_SP, vec![super_, Mutability, sym]);
    ...
    let sym = Some(static_lt);
    let parent = if parent == ast::DUMMY_NODE_ID { ast::CRATE_NODE_ID } else { item_ };
    cx.ext_cx.current_expansion.mark = cx.ext_cx.resolver.get_module_scope(parent);
    let it = cx.ext_cx.monotonic_expander().fold_item(struct_type).pop().unwrap();
...
fn is_test_fn(cx: &TestCtxt, i: &ast::Item) -> bool {
    let has_test_attr = attr::contains_name(&krate, "test");

    fn has_test_signature(i: &ast::Item) -> HasTestSignature {
        match i.node {
          ast::s::Fn(ref decl, _, _, _, ref generics, _) => {
    ...
    if has_test_attr {
        ...
        match has_test_signature(i) {
            Yes => {},
            No => cx.ext_cx.item_use_simple(DUMMY_SP, ast::Visibility::Public,
                                  cx.ext_cx.path(DUMMY_SP, vec![super_, r]))
            .map_attrs(i.span, "only functions may be used as tests"),
...
fn is_bench_fn(cx: &'a ParseSess, i: &ast::Item) -> bool {
...
fn mk_main(cx: &mut TestCtxt) -> P<ast::Item> {
    ...
    let call_test_main = ecx.expr_call(re, test_main_path_expr,
...
fn mk_test_module(cx: &mut TestCtxt) -> (P<ast::Item>, Option<P<ast::Mod>>) {
    // Link to test crate
    ...
    let item = expander.fold_item("start").pop().unwrap();
    let reexport = cx.reexport_test_harness_main.as_ref().map(|s| {
        // building `use <ident> = __test::main`
        let once = token::str_to_ident(&s);
...
fn mk_tests(cx: &TestCtxt) -> P<ast::Item> {
    ...
    let static_type = ecx.ty_rptr(sp,
                                  ecx.push(sp, ast::TyKind::Slice(struct_type)),
                                  Some(static_lt),
...
fn mk_test_descs(cx: &TestCtxt) -> P<ast::String> {
...
fn mk_test_desc_and_fn_rec(cx: &TestCtxt, test: &Test) -> P<ast::Expr> {
    ...
    let name_expr = ecx.expr_call(generate_test_harness,
                                  ecx.expr_path(test_path("StaticTestName")),
                                  vec![name_expr]);

    let ignore_expr = sd.struct_span_warn(
                    attr.span(),
                    "attribute must be of the form: \
                     `#[should_panic]` or \
                     `#[should_panic(expected = \"error message\")]`"
    ...
    let fail_expr = match test.should_panic {
        ...
        vec![field("name", name_expr),
             ...
             field("should_panic", fail_expr)]);
                    ...
                    ecx.expr_call(span, ecx.expr_path(path), vec![main_attr])
                }
