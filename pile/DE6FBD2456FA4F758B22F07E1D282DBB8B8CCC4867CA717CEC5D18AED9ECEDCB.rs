...
fn should_accept_identity() {
    ...
    assert::is_transmutable::<  i32,   f64>();
...
fn should_reject_extension() {
    ...
    assert::B::<   u8,  u128>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  i64,  u128>(); //! ALL valid bit patterns of the source must be valid bit patterns of the
    assert::is_transmutable::<  i64,  i128>(); //~ ERROR cannot be safely transmuted

    assert::is_transmutable::<duplex, array>(); //~ ERROR cannot be safely transmuted
