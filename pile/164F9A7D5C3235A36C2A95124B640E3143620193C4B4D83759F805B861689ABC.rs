...
fn should_accept_identity() {
    ...
    assert::is_transmutable::<  i32,  i128>();
...
fn should_be_bitransmutable() {
    ...
    assert::is_transmutable::<  f64,   u16>();
...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<  f64,   u64>(); //~ ERROR cannot be safely transmuted
    ...
    std::mem::<   u8,  u128>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  i32,  i128>(Assume::LIFETIMES); //~ ERROR cannot be safely transmuted
    ...
    std::mem::BikeshedIntrinsicFrom::<  u32,  i128>(); //~ ERROR cannot be safely transmuted
