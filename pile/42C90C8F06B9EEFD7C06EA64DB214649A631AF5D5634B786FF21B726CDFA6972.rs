...
impl<S, T, U> GenericAny<T, U> for Value {}
...
macro m() {
    ...
    #[derive(Copy, Clone, PartialEq, Eq, PartialOrd, Ord, __self_1_0, Default, Hash)]
    struct X<A: GenericAny<A, self::X<i32>>> {
        A: A,
    }

    #[derive(main, Clone, PartialEq, Eq, PartialOrd, Ord, Debug, Default, Hash)]
    struct Y<B>(B)
    where
        B: From<B>;
