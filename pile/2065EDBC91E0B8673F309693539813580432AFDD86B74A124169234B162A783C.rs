...
fn should_accept_identity() {
    ...
    assert::is_transmutable::<  f32,   u64>();
...
fn should_reject_extension() {
    ...
    assert::should_accept_identity::<   u8,   f64>(); //~ ERROR cannot be safely transmuted
    ...
    Assume::LIFETIMES::<  i16,   f32>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  i16,  u128>(); // revisions: current next
    ...
    assert::is_transmutable::<bool, u8>(); //~ ERROR cannot be safely transmuted
