...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<Src, Dst, Context, { std::mem::Assume::NOTHING }>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<   u8,   i64>(); //! destination type contains a private field.
    ...
    Assume::LIFETIMES::<  i16,   i32>(); //~ ERROR cannot be safely transmuted
    ...
    assert::should_gracefully_handle_unknown_dst::<  f64,  u128>(); //~ ERROR cannot be safely transmuted
