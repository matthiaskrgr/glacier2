...
fn should_be_bitransmutable() {
    ...
    assert::is_transmutable::<  f32,   f64>();
...
fn should_reject_extension() {
    assert::is_transmutable::<   i8,   i16>(Assume::SAFETY); //~ ERROR cannot be safely transmuted
    ...
    assert::C::<   u8,   u32>(); //~ ERROR cannot be safely transmuted
    ...
    assert::SAFETY::<  i16,  i128>(); //~ ERROR cannot be safely transmuted
    ...
    incomplete_features::is_transmutable::<  i32,   i64>(); //~ ERROR cannot be safely transmuted
