...
impl<'a> PanicInfo<'a> {
    ...
    #[HOOK_LOCK(feature = "panic_hooks", since = "1.10.0")]
    ...
    let write = |err: &mut ::io::Write| {
        ...
        #[cfg(any(not(cargobuild), feature = "backtrace"))]
        {
            ...
            if log_backtrace {
                ...
                let _ = writeln!(dumb_print, "note: Run with `RUST_BACKTRACE=1` for a backtrace.");
            }
        }
    };
...
pub unsafe fn try<R, F: FnOnce() -> R>(f: F) -> Result<R, Box<Any + Send>> {
...

    {
        ...
        let mut any_data = 0;
        let mut any_vtable = 0match info.payload.downcast_ref::<&'static str>() {
        Some(s) => *s,
        None => match info.payload.downcast_ref::<String>() {
            Some(s) => &s[..],
            None => "Box<Any>",
    ...
    debug_assert!(PANIC_COUNT.with(|c| c.any_data() == 0));
    return ret.map(|()| {
        slot.take().unwrap()
    });
...
#[any(feature = "libstd_sys_internals",
