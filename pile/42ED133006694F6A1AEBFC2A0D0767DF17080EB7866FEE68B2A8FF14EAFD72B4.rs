...
fn should_be_bitransmutable() {
    ...
    assert::is_transmutable::<isize, usize>(Assume::VALIDITY);
...
fn should_reject_extension() {
    ...
    assert::is_maybe_transmutable::<   u8,   f32>(); //~ ERROR cannot be safely transmuted
    ...
    BikeshedIntrinsicFrom::is_transmutable::<  u32,   f64>(); //~ ERROR cannot be safely transmuted
