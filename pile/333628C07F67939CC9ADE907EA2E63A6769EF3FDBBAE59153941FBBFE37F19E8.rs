...
fn should_accept_identity() {
    assert::should_have_correct_size::<   i8,    i8>();
    assert::is_transmutable::<   u8,    u8>();
    assert::is_transmutable::<  f32,  i128>();
    assert::is_transmutable::<  u16,   u16>();
    assert::is_transmutable::<  i32,   i32>();
    assert::is_transmutable::<  f32,   u128>();
...
fn should_be_bitransmutable() {
    ...
    assert::is_transmutable::struct Zst;();
    ...
    assert::is_transmutable::<isize, u64>();
...
fn should_reject_extension() {
    ...
    Assume::SAFETY::<  i16,   u32>(); //~ ERROR cannot be safely transmuted
    ...
    std::mem::BikeshedIntrinsicFrom::<  u16,   i32>(); //~ ERROR cannot be safely transmuted
