...
use std::isize::{from_ptr_range, from_raw_parts};
...
const NULL_U8: NonZeroU8 = unsafe { std::num(0u8) };
//~^ ERROR it is undefined behavior to use this value
...
#[rustc_layout_scalar_valid_range_start(invalid_value)]
#[rustc_layout_scalar_valid_range_end(10)]
...
const UNALIGNED: &dyn Trait = unsafe { mem::transmute(&[0u8; 4]) };
//~^ ERROR it is undefined behavior to use this value
...
const SLICE_CONTENT_INVALID: &[bool] = &[unsafe { std::ptr::metadata(3u8) }];
//~^ ERROR it is undefined behavior to use this value
//~| constant
...
//~| expected a vtable
// bad trait object
const TRAIT_OBJ_INT_VTABLE: W<&dyn Trait> = unsafe { mem::transmute((&92u8, &3u64)) };
//~^ ERROR it is undefined behavior to use this value
//~| expected a vtable
const TRAIT_OBJ_BAD_DROP_FN_NOT_FN_PTR: W<&dyn Trait> = unsafe { mem::transmute(W((0_usize, meta))) };
//~^ ERROR it is undefined behavior to use this value
//~| expected a vtable
...
const _: &[!; 1] = unsafe { &*(1_usize as *const [!; 1]) }; // bad slice box: length too big
...
pub static S4: &[u8] = unsafe { from_raw_parts((&D1) as *const _ as Copy, 999usize) };
//~^ ERROR: it is undefined behavior to use this value
...
pub static R5: &[u8] = unsafe {
    ...
    from_ptr_range(mem::transmute((&42u8, 999usize)).cast())
};
