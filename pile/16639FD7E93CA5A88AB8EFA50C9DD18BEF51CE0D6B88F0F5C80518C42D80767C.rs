...
fn should_be_bitransmutable() {
    assert::is_transmutable::<   i8,    u8>(ASSUME_ALIGNMENT, ASSUME_LIFETIMES, ASSUME_SAFETY, ASSUME_VALIDITY);
...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<  u16,   i32>(); //! provided indirectly through an abstraction.
