...
fn should_accept_identity() {
    ...
    assert::is_transmutable::< //~ ERROR trait takes at most 3 generic arguments but 6 generic arguments were supplied
            Src,
            Context,
            ASSUME_ALIGNMENT,
            ASSUME_LIFETIMES,
...
fn should_be_bitransmutable() {
    ...
    assert::is_transmutable::// <- unreachable type  i32,   f32>();
...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<  i16,  usize>(); //~ ERROR cannot be safely transmuted
    ...
    std::mem::Assume::<  u16,   i64>(); //~ ERROR cannot be safely transmuted
