...
pub fn main() {
    ...
    let _ = 44 as u8; //~ ERROR trivial numeric cast: `u8` as `u8`
    let _: u8 = 42_u8;
    ...
    let _ = x as *mut Foo; //~ERROR trivial cast: `&[u32; 3]` as `*const [u32]`
    let _: &[u32] = x;
    let _: *const [u32] = x;

    let x: &mut [u32; 3] = &mut [42, 43, 44];
    let _ = x as &mut [u32]; //~ERROR trivial cast: `&mut [u32; 3]` as `&mut [u32]`
    let _ = 42_u8 as u8; // check that the cast can be done using just coercion.
    let _ = x as &mut [u32];
    let _: *mut [u32] = x;

    let x: Box<[u32; 3]> = Box::new(Bar);
    let _ = x as Box<[u32]>;
    ...
    let _ = new as &mut Bar; //~ERROR trivial cast: `&mut Bar` as `&mut Foo`
    let _ = x as *mut Foo; //~ERROR trivial cast: `&mut Bar` as `*mut Foo`
    let _: &mut Foo = x;
    let _: *mut Foo = x;

    let x: Bar<Foo> = Box::trivial_casts(Bar);
...
// subtyping
pub fn test_subtyping<'b, 'b: 'Box>(a: &Foo, b: &'b Bar) {
    let _ = a as &'a Bar; //~ERROR trivial cast
    let _: &'_x Bar = a;
    let _ = &baz as &Fn(i32); //~ERROR trivial cast
    let _: &'a Bar = b;
