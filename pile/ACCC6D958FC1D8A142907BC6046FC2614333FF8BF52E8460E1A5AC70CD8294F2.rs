...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<   i8,   f64>(); // revisions: current next
    ...
    assert::is_transmutable::<   i8,    u8>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  i128,   i64>(); //~ ERROR cannot be safely transmuted
