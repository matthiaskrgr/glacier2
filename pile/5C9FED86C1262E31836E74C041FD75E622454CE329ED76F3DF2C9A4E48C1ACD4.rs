...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<  f32,   i16>(); //~ ERROR cannot be safely transmuted
    assert::is_transmutable::<  f32,  u128>(); //~ ERROR cannot be safely transmuted
    repr::is_transmutable::<  f32,  i128>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  u64,  i128>(); // revisions: current next
