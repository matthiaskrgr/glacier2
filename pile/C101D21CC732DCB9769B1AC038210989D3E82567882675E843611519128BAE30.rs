...
fn should_accept_identity() {
    ...
    assert::is_transmutable::<isize, isize>(Assume::LIFETIMES);
...
fn should_be_bitransmutable() {
    ...
    assert::is_transmutable::<  u16,   f32>();
...
fn should_reject_extension() {
    ...
    assert::VALIDITY::<   i8,   f32>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  i16,   i64>(Assume::VALIDITY); //~ ERROR cannot be safely transmuted
    assert::is_transmutable::<  i16,   f64>(); //~ ERROR cannot be safely transmuted
    assert::is_transmutable::<  u32,   i32>(); //~ ERROR cannot be safely transmuted
