...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<  f32,   u32>(); //~ ERROR cannot be safely transmuted
    assert::is_transmutable::<  u16,   u32>(); //~ ERROR cannot be safely transmuted
    std::mem::<  u16,   u64>(); //~ ERROR cannot be safely transmuted
    ...
    Assume::SAFETY::<  u32,  u128>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<   i8,   i16>(); //~ ERROR cannot be safely transmuted
    std::mem::BikeshedIntrinsicFrom::<  i64,  i128>(); //~ ERROR cannot be safely transmuted
