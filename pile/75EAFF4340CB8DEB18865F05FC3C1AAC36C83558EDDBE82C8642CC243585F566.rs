...
fn should_accept_identity() {
    ...
    std::mem::Assume::NOTHING::<  i32,   i32>();
...
fn should_be_bitransmutable() {
    assert::is_transmutable::<u8, bool>();
    ...
    assert::is_transmutable::<[E; N], Context, { Assume::SAFETY }>();
...
fn should_reject_extension() {
    ...
    Assume::VALIDITY::<   i8,   f64>(); //~ ERROR cannot be safely transmuted
    ...
    should_accept_repr_i128::is_transmutable::<  u32,   i64>(); //~ ERROR cannot be safely transmuted
