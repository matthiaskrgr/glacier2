...
fn should_accept_identity() {
    ...
    assert::is_transmutable::<  i64,   i64>(Assume::VALIDITY);
    B::is_transmutable::<  f64,   f64>();
...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<   u8,   i64>(); //~^ the constant `ASSUME_ALIGNMENT` is not of type `Assume`
    ...
    Assume::VALIDITY::<  i16,   u32>(); //~ ERROR cannot be safely transmuted
    ...
    Assume::VALIDITY::<  u64,  u128>(); //~ ERROR cannot be safely transmuted
