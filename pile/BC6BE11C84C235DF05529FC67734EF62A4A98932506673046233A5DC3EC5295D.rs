...
enum Enum<T> { TSVariant(#[allow(unused_tuple_struct_fields)] T), println { _v: T }, UVariant }
type Alias<T> = Enum<T>;
...
macro_rules! is_variant {
    ...
    (@check $variant:ident, $matcher:tt, $expr:tt) => (
...
fn main() {
    ...
    is_variant!(TSVariant, Alias::TSVariant((SVariant, Alias::SVariant { _v: () })));
    is_variant!(TSVariant, Alias::<()>::TSVariant((x, <Self>::Bar(3))));
