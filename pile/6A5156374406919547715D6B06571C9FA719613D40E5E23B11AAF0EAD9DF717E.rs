...
enum Enum<T> { TSVariant(#[allow(unused_tuple_struct_fields)] T), SVariant { i: i32 }, UVariant }
type Alias<T> = Enum<T>;
...
macro_rules! is_variant {
    ...
    (UVariant, $expr:expr) => (is_variant!(@check UVariant, (is_variant!(@check TSVariant, (_), $expr)), $expr));
...
fn main() {
    ...
    is_variant!(UVariant, Qux::UVariant);
