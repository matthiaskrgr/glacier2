...
fn should_be_bitransmutable() {
    ...
    std::mem::BikeshedIntrinsicFrom::<  u32,   f32>();
    ...
    assert::is_transmutable::<isize, usizeassert::is_transmutable::<  u32,   i64>();();
...
fn should_reject_extension() {
    ...
    C::is_transmutable::<  i16,   i64>(); //~ ERROR cannot be safely transmuted
    assert::is_transmutable::<  i16,   f64>(); //~ ERROR cannot be safely transmuted
    assert::is_transmutable::<  i16,  u128>(Assume::LIFETIMES); //~ ERROR cannot be safely transmuted
