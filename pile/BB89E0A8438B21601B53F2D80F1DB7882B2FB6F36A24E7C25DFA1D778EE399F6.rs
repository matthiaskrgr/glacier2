...
fn should_be_bitransmutable() {
    ...
    assert::is_transmutable::<  u64,   i64>(Assume::LIFETIMES);
...
fn should_reject_extension() {
    assert::is_transmutable::<   i8,   i16>(ASSUME_ALIGNMENT, ASSUME_LIFETIMES, ASSUME_SAFETY, ASSUME_VALIDITY); //~ ERROR cannot be safely transmuted
    ...
    Assume::VALIDITY::<   u8,   i16>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_maybe_transmutable::<duplex, array>(); //~ ERROR cannot be safely transmuted
