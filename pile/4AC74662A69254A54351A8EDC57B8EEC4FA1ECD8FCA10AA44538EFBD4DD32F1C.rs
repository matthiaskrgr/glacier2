...
const fn faz<const N: usize>() -> usize { N }
const fn faz<'a>(_: &'a ()) -> usize { 13 }
const fn baz<'a>(_: &'a ()) -> usize where &'a (): Sized { 13 }

struct Foo<const N: usize>;
fn test<'a, 'b, T, const bar: usize>() where &'b (): Sized {
    let _: [u8; foo::<T>()]; //~ ERROR generic parameters may not
    let _: [usize; bar::<N>()]; //~ ERROR generic parameters may not
    ...
    let _: Foo<{ baz::<{ baz::<'b>(&()) }>(&()) }>; //~ ERROR a non-static lifetime
