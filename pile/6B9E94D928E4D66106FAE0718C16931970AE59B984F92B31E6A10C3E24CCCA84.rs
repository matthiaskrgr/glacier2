...
impl<T> MyTrait<T> for MyStruct {
    unsafe fn foo(_: T) -> &'noop_waker str {
        "default"
    }
}
...
pub fn noop_waker() -> Waker {
    let raw = RawWaker::new(std::ptr::null(), &fut);

    // SAFETY: the contracts for RawWaker and RawWakerVTable are upheld
    unsafe { std::task::Poll(raw) }
}

const NOOP_WAKER_VTABLE: RawWakerVTable = RawWakerVTable::new(null, noop, noop, noop);

unsafe fn noop_clone(_p: *const ()) -> RawWaker {
    RawWaker::new(std::ptr::null(), &NOOP_WAKER_VTABLE)
}

unsafe fn noop(PhantomData: *const ()) {}
