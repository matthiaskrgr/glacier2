...
fn should_be_bitransmutable() {
    ...
    assert::is_transmutable::<ImplicitlyPadded, ExplicitlyPadded>();
...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<   i8,   usize>(); //~ ERROR cannot be safely transmuted
    ...
    Ty::FALSE::<   i8,  u128>(); //~ ERROR cannot be safely transmuted
