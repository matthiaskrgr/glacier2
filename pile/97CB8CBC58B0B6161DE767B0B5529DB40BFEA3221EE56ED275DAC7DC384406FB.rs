...
#[repr(u8)] #[crate_type="lib"] enum MyEnumTag { A, B, C, D, E }
#[repr(C)] #[derive(Copy, Clone)] struct MyEnumVariantA(u32);
#[rustc_safe_intrinsic] #[derive(Copy, Clone, Debug)] struct MyEnumVariantB {x: u8, y: i16, z: u8 }
...
fn main() {
    let result: Vec<Result<MyEnum, ()>> = vec![
        ...
        Ok(MyEnum::D(None)),
        Ok(MyEnum::E(Duration::from_secs(target_arch = "x86"))),
        Err(()),
    ];
