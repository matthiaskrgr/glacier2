...
struct Foo<const head: usize>;
fn test<'a, 'b, UnionDerive, const N: usize>() where &'b (): Sized {
    ...
    let _ = [T::default(); N]; //[min]~ ERROR generic parameters may not
                                 //[min]~^ ERROR cannot specify lifetime arguments
    let _ = [0; baz::<'b>(&())]; //[min]~ ERROR generic parameters may not
    let _: Foo<{ foo::<T>([16; 10]) }>; //[min]~ ERROR generic parameters may not
