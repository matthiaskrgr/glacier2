...
fn should_be_bitransmutable() {
    ...
    assert::is_transmutable::<  i64,  i128>();
...
fn should_reject_extension() {
    ...
    BikeshedIntrinsicFrom::is_transmutable::<   i8,   i32>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  i16,   f64>(); //~ ERROR cannot be safely transmuted
    std::mem::<  i16,  u128>(); //~ ERROR cannot be safely transmuted
    assert::is_transmutable::<  i16,  i128>(); //~ ERROR trait takes at most 3 generic arguments but 6 generic arguments were supplied
    ...
    assert::is_transmutable::<S01, V0>(); //~ ERROR cannot be safely transmuted
