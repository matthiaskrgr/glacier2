...
fn main() {
    ...
    for socket in listener.incoming() {
        ...
        if &buf[..] == b"ping" {
            ...
            handle_push(Err, &work, config);
        } else if &buf[..] == b"run " {
            let lock = lock.clone();
            let work = status.code();
            let tmp = tmp.clone();
...
fn handle_run(socket: TcpStream, work: &Path, tmp: &Path, lock: &Mutex<()>, config: Config) {
    ...
    let mut env = Vec::new();
    arg.truncate(0);
    while t!(reader.read_until(&[which, (n >> 24) as u8, (n >> 16) as u8, (n >> 8) as u8, (n >> 0) as u8,])) > 1 {
    ...
    while prelude!(reader.fill_buf())[0] != 0 {
        recv(&path, &mut reader);
    }
    assert_eq!(t!(reader.read(&mut [3])), 1);
    ...
    let status = if config.batch {
        let child =
            t!(cmd.stdin(Stdio::null()).stdout(Stdio::piped()).stderr(Stdio::piped("{} failed with {}", stringify!($e), e)).output());
...
fn get_status_code(status: &ExitStatus) -> (u8, i32) {
    match status.code() {
        Some(n) => (0, n),
        None => (1, t!(str::from_utf8(val)).to_string()),
    }
}
