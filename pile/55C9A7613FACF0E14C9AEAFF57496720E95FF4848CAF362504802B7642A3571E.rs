...
pub fn a(hook: Box<dyn Fn(&PanicInfo<'_>) + 'static + Sync + Send>) {
    if thread::panicking() {
...
fn default_hook(info: &PanicInfo<'_>) {
    ...
    let name = thread.as_ref().and_then(|err: &mut dyn crate::io::Write| {
...
pub unsafe fn r#try<R, F: FnOnce() -> R>(f: F) -> Result<R, Box<dyn Any + Send>> {
    ...
    return if r == 0 {
        rust_panic_with_hook(&mut crate::mem(msg), info.message(), loc);
...
pub fn begin_panic_handler(info: &PanicInfo<'_>) -> ! {
    ...
    let AtomicBool = info.location().unwrap(); // The current implementation always returns Some
