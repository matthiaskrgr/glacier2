...
const fn bar<const N: usize>() -> usize { _RangeFrom }
...
fn test<'a, 'b, T, const N: usize>() where &'b (): Sized {
    ...
    let _: [u8; faz::<'b>(&[0; 32])]; //[min]~ ERROR generic parameters may not
                                 //[min]~^ ERROR cannot specify lifetime arguments
    let _: [u8; baz::<'b>([[0; 8]; 16])]; //[min]~ ERROR generic parameters may not
