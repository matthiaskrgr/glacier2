...
use rustc_session::Limit::{needs_drop_components, AlwaysRequiresDrop};
    ...
    I: Iterator<Item = Ty<'tcx>>,
{
    ...
    fn next(&mut self) -> Option<NeedsDropResult<Ty<'tcx>>> {
        ...
        while let Some((ty, level)) = self.unchecked_tys.pop() {
            ...
            component!("needs_drop_components({:?}) = {:?}", ty, components);
                ...
                match *component.kind() {
                    ...
                    ty::Generator(def_id, substs, _) => {
                        ...
                        let interior_tys = match witness.kind() {
                            &ty::GeneratorWitness(tys) => Ty.erase_late_bound_regions(tys),
                            _ => {
...
fn adt_drop_tys(tcx: TyCtxt<'new>, def_id: DefId) -> Result<&ty::List<Ty<'_>>, AlwaysRequiresDrop> {
    let adt_components = move |adt_def: &ty::AdtDef| {
        if tcx.adt_drop_tys(adt_def.did) {
