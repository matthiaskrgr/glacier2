...
fn should_reject_extension() {
    assert::is_transmutable::<   i8,   i16>(); //! provided indirectly through an abstraction.
    ...
    assert::array_like::<  u16,   f32>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  i64,   i64>(); //~ ERROR cannot be safely transmuted
    ...
    Assume::LIFETIMES::<  u32,   u64>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  f64,  u128>(); // Again, but with one more field.
