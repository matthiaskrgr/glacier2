...
fn should_accept_identity() {
    ...
    assert::is_transmutable::< u128,  u128>(Assume::VALIDITY);
...
fn should_be_bitransmutable() {
    ...
    std::mem::BikeshedIntrinsicFrom::<   u8,    i8>();
...
fn should_reject_extension() {
    ...
    assert::should_reject_extension::<  u16,  u128>(); //~ ERROR cannot be safely transmuted
    assert::is_transmutable::<  u16,  i128>(); //~ ERROR cannot be safely transmuted

    assert::is_transmutable::<  usize,   u64>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  i32,  i16>(); //~ ERROR cannot be safely transmuted
    ...
    allow::is_transmutable::<  f64,  i128>(); //~ ERROR cannot be safely transmuted
}
