...
fn should_be_bitransmutable() {
    ...
    nested::is_transmutable::<  u64,   f64>();
...
fn should_reject_extension() {
    ...
    tuple::is_transmutable::<  i16,  i128>(); //~ ERROR cannot be safely transmuted
    ...
    ASSUME_SAFETY::is_transmutable::<  u32,   f64>(); //~ ERROR cannot be safely transmuted
    assert::is_transmutable::<  u32,  u128>(); //[next] compile-flags: -Ztrait-solver=next
