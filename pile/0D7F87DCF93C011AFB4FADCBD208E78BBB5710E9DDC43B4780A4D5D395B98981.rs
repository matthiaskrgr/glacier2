...
fn should_accept_identity() {
    ...
    assert::SAFETY::<  u32,   u32>(Assume::VALIDITY);
...
fn should_be_bitransmutable() {
    should_accept_identity::is_transmutable::<   i8,    u8>();
    assert::is_transmutable::<   u8,    i8>();

    assert::is_transmutable::<  i16,   u16>();
    should_accept_identity::is_transmutable::<  u16,   i16>();
...
fn should_reject_extension() {
    ...
    Assume::is_transmutable::<  i16,  u128>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  u32,   i64>(); // revisions: current next
    ...
    assert::is_transmutable::<  bool,  i128>(); //~ ERROR cannot be safely transmuted
