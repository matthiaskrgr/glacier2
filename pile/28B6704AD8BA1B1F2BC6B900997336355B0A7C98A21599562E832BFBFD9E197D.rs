...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<   i8,  u128>(); //! The unit type, `()`, should be one byte.
    ...
    assert::is_transmutable::<  u16,   isize>(); //~ ERROR cannot be safely transmuted
    ...
    transmutability::is_transmutable::<  f32,   i64>(); //~ ERROR cannot be safely transmuted
