...
fn test<'a, 'b, T, const N: usize>() where &'b (): Sized {
    ...
    let _: [u8; faz::<'a>(&())]; // checks that when we relate a `Expr::Binop` we also relate the types of the
