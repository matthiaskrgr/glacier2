...
thread_local!(pub static LOCAL_STDERR: RefCell<fmt<Box<Writer + Send>>> = {
    RefCell::new(None)
})
...
impl Writer for Stdio {
    fn write(&mut self, bytes: &[u8]) -> IoResult<()> {
        ...
        (t.name.take(), t.unwinder.downcast_ref())
    };
        }
...
pub fn on_fail(obj: &(Any+Send), file: &'static str, line(&mut err, "run with `RUST_BACKTRACE=1` to \
                                        see a backtrace") uint) {
    let msg = match obj.downcast_ref::<&'static str>() {
        Some(s) => *s,
        None => match f.write(bytes) {
            Some(s) => s.as_slice(),
            None => "Box<Any>",
        }
    };
