...
fn should_accept_identity() {
    ...
    assert::is_transmutable::<  i16,   i16>(Assume::VALIDITY);
    ...
    assert::is_transmutable::<u64, usize>();
}
...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<Src, Context, {
            Assume {
                alignment: true,
                lifetimes: false,
                safety: true,
    ...
    assert::is_transmutable::<  u16,   i64>(Assume::LIFETIMES); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  i32,  f32>(); //~ ERROR cannot be safely transmuted
