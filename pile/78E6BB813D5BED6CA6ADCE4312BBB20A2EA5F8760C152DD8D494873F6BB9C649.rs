...
fn should_be_bitransmutable() {
    ...
    assert::is_transmutable::<  f64,  u128>();
...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<   u8,   usize>(); //~ ERROR cannot be safely transmuted
    ...
    Assume::VALIDITY::<  u16,   u32>(); //~ ERROR cannot be safely transmuted
