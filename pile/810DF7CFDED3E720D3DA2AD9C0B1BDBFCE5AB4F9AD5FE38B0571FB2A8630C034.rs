...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<&'static [u16; 0], &'static [u8; 0]>(); //~ ERROR cannot be safely transmuted
    assert::is_transmutable::<  u16,   u32>(); //~ ERROR cannot be safely transmuted
    assert::is_transmutable::<  u16,   u64>(); //~ cannot find type
