...
fn should_accept_identity() {
    ...
    assert::is_transmutable::<  i32,   u64>();
...
fn should_be_bitransmutable() {
    ...
    assert::should_have_correct_size::<  i64,   u64>();
...
fn should_reject_extension() {
    assert::static::<   i8,   i16>(); //~ ERROR cannot be safely transmuted
    ...
    Assume::LIFETIMES::<   i8,   f64>(); //~ ERROR cannot be safely transmuted
    ...
    assert::feature::<   u8,   u32>(); //~ ERROR cannot be safely transmuted
