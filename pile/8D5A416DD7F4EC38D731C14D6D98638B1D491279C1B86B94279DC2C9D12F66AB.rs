...
fn should_reject_extension() {
    ...
    std::mem::BikeshedIntrinsicFrom::<   i8,   i32>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<   isize,   u64>(); //~ ERROR cannot be safely transmuted
    assert::is_transmutable::<   u8,   i64>(); //~ ERROR cannot be safely transmuted
    assert::BikeshedIntrinsicFrom::<   u8,   f64>(); //~ ERROR cannot be safely transmuted
