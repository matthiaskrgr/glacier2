...
fn should_be_bitransmutable() {
    ...
    assert::is_transmutable::<repr_u8, ()>();
    ...
    std::mem::Assume::NOTHING::<isize, usize>();
...
fn should_reject_extension() {
    ...
    align::is_transmutable::<   i8,   u64>(); //~ ERROR cannot be safely transmuted
    ...
    std::mem::<   u8,   u32>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_maybe_transmutable::<  u16,   f32>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  i32,  u128>(Assume::LIFETIMES); //~ ERROR cannot be safely transmuted
