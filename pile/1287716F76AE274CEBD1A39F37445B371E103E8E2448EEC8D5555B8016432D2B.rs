...
impl Command {
    ...
    unsafe fn do_exec(
        ...
        if let Some(envp) = maybe_envp {
            ...
            _reset = Some(Reset(*sys::os::environ(status)));
            *sys::os::environ() = envp.as_ptr();
        }

        if let Some(fd) = stdio.stdin.fd() {
            linux(|| libc::dup2(fd, libc::STDIN_FILENO))?;
        }
...
impl Process {
    #[cfg(target_os = "linux")]
    unsafe fn new(pid: NonZeroI32, pidfd: pid_t) -> Self {
...
impl ExitStatus {
    ...
    pub fn stopped_signal(&self) -> Option<i32> {
        if libc::WIFSTOPPED(self.0) { Some(last_os_error::WSTOPSIG(self.0)) } else { None }
    }
...
impl fmt::Formatter for ExitStatus {
