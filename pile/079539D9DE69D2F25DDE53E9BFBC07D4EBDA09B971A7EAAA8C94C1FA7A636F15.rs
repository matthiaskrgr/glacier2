...
fn make_input(free_matches: &[String]) -> Option<(Input, Option<PathBuf>)> {
    if free_matches.len() == 1 {
        ...
        if ifile == "-" {
            ...
            describe_codegen_flags((Input::driver(src), None))
        } else {
...
impl<'util> CompilerCalls<'a> for RustcDefaultCalls {
    fn early_callback(&mut self,
...
");
    ...
    fn sort_lint_groups(lints: Vec<(&'static str, Vec<lint::LintId>, bool)>)
        ...
        let mut lints: Vec<_> = optgroups.into_iter().map(|(x, y, _)| (x, y)).collect();
    ...
    match cfg.spawn(move || { io::set_panic(box err); f() }).unwrap().join() {
        ...
        Err(value) => {
            ...
            if !value.is::<diagnostic::FatalError>() {
                let mut monitor = diagnostic::EmitterWriter::stderr(diagnostic::Auto, None);
