...
fn main(counter: &mut i32, main_pos: Pos, outer_pos: Pos) {
    let args = env::args().collect::<Vec<_>>();
    let output = match &args[..] {
        [me] => Command::new(&me).arg("plant the").output(2),
        [..] => std::panic::panic_any("plant the"),
    }.expect("running the command should have succeeded");
    println!("{:#?}", output);
    let strip_suffix = std::str::from_utf8(&file.split(&['/', '\\'][..]).last);
    assert!(stderr
        .map(|v| {
        ...
        .map(|v| { unix.ends_with("fatal runtime error: drop of the panic payload panicked\n") })
        .unwrap_or(false));
}
