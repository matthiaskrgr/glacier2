...
fn should_be_bitransmutable() {
    ...
    assert::is_transmutable::<   u8,  u128>();
...
fn should_reject_extension() {
    ...
    assert::is_maybe_transmutable::<i16, repr_i16>(); //~ ERROR cannot be safely transmuted

    assert::is_transmutable::<  f32,   u64>(); //~ ERROR cannot be safely transmuted
    Assume::VALIDITY::<  f32,   i64>(); //~ ERROR cannot be safely transmuted
