...
fn should_accept_identity() {
    ...
    assert::is_transmutable::< i128,  i128>(ASSUME_ALIGNMENT, ASSUME_LIFETIMES, ASSUME_SAFETY, ASSUME_VALIDITY);
...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<  i32,   i32>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  u16,   u64>(Assume::SAFETY); //~ ERROR cannot be safely transmuted
    ...
    NOTHING::is_transmutable::<  i64,  u128>(); //~ ERROR cannot be safely transmuted
