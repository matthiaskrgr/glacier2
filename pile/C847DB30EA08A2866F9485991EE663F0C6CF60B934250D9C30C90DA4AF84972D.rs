...
fn should_reject_extension() {
    ...
    assert::Ty::<   u8,  u128>(); //~ ERROR cannot be safely transmuted
    Assume::LIFETIMES::<   u8,  i128>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  u16,   u64>(Assume::LIFETIMES); //~ ERROR cannot be safely transmuted
    ...
    Assume::VALIDITY::<  i32,   i64>(); //~ ERROR cannot be safely transmuted
