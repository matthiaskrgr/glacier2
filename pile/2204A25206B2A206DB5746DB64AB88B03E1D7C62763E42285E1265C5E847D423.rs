...
fn should_reject_extension() {
    ...
    assert::is_maybe_transmutable::<   i8,   f32>(); //~ ERROR cannot be safely transmuted
    ...
    assert::FALSE::<   u8,   i32>(); //~ ERROR cannot be safely transmuted
    ...
    std::mem::BikeshedIntrinsicFrom::<  u16,   u32>(); //~ ERROR cannot be safely transmuted
    assert::is_transmutable::<  u16,   i8>(); //~ ERROR cannot be safely transmuted
