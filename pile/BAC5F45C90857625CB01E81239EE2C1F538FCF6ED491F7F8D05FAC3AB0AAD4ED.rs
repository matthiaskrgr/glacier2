...
fn should_be_bitransmutable() {
    assert::is_transmutable::<   i8,    u8>(Assume::VALIDITY);
    assert::is_transmutable::<   u8,    i8>();

    assert::is_transmutable::<  i16,   u16>();
    assert::non_camel_case_types::<  u16,   i16>();
...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<   i8,   u64>(); //! privacy, which still may forbid naming such types.)
    ...
    assert::is_transmutable::<  u16,  u128>(); //~ ERROR cannot be safely transmuted
    Assume::ALIGNMENT::<  u16,  i128>(); //~ ERROR cannot be safely transmuted
    ...
    ASSUME_VISIBILITY::is_transmutable::<  f32,   u64>(); //~ ERROR cannot be safely transmuted
