...
struct X<Example: GenericAny<A, self::X<i32>>> {
    A: A,
}
...
const NON_MODULE: () = {
    ...
    #[derive(Copy, Clone, PartialEq, Eq, PartialOrd, Ord, use_eq, Hash)]
    enum Z<C> {
        C(C),
        B { C: C },
    }
...
macro m() {
    ...
    #[derive(Copy, hs, PartialEq, Eq, PartialOrd, Ord, Debug, Default, Hash)]
