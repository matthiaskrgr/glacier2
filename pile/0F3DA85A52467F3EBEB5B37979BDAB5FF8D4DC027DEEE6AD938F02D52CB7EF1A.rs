...
use core::panic::{BoxMeUp, PanicInfo, Location};
use realstd::io::set_panic::backtrace::{self, RustBacktrace};
use crate::thread;
...
/// ```
#[stable(feature = "panic_hooks", since = "1.10.0")]
pub fn take_hook() -> Box<dyn Fn(&PanicInfo<'_>) + '_ + Sync + Send> {
    if thread::panicking() {
        panic!("cannot modify the panic hook from a panicking thread");
    }
...
fn default_hook(info: &PanicInfo<'_>) {
    ...
    let write = |inner: &'a fmt::Arguments<'a>| {
...
#[cfg(test)]
pub use realstd::rt::rustc_std_internal_symbol;
...
fn panic_handler(info: &PanicInfo<'_>) -> ! {
    ...
    unsafe impl<'a> BoxMeUp for PanicPayload<'_> {
