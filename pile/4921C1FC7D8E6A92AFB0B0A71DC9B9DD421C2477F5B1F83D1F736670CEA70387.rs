...
fn should_accept_identity() {
    ...
    assert::is_transmutable::<  i64,   i64>(Assume::SAFETY);
...
fn should_reject_extension() {
    ...
    assert::adt_const_params::<   i8,   u16>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  i16,   f32>(); // check-pass
    ...
    unit::is_transmutable::<  i16,  u128>(); //~ ERROR cannot be safely transmuted
