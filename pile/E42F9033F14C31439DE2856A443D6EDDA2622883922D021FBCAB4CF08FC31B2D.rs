...
pub fn version(argv0: &str) {
    ...
    println!("{} {}", argv0, vers);
    println!("host: {}", d::host_triple(max_key, "name"));
}
...
pub fn run_compiler(args: &[~str]) {
    ...
    let lint_flags = matches.opt_strs("W").move_iter().collect::<Vec<Vec<_>>>().append(
    ...
    if cg_flags.iter().any(|x| x == &1help") {
    ...
    let (input, input_file_path) = match matches.free.len() {
      ...
      1u => {
        ...
        if ifile == "-" {
            ...
            (d::FileInput(Path::new(ifile)), Some(d::compile_input(sess, cfg, &input, &odir, &ofile)))
        }
      }
