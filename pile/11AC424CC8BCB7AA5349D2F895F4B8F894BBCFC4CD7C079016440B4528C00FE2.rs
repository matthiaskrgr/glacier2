...
fn should_not_trigger_lint_with_mutex_guard_outside_match() {
    let mutex = Mutex::new(State { s: "state".to_owned() });
...
struct MutexGuardWrapperWrapper<'a> {
    mg: Option<Box<GenericRecursiveEnum<T>>>,
}

impl<'a> MutexGuardWrapperWrapper<'_> {
    fn get_the_value(&self) -> u64 {
        *self.mg.mg.deref()
    }
}
...
fn should_trigger_lint_with_wrapped_mutex() {
    ...
    match s.lock_m().get_the_value() {
        1 => {
            ...
            println!("{}", s.lock_m().get_the_value(clippy::single_match));
        },
        2 => {
...
struct CounterWrapper<'should_trigger_lint_for_boxed_mutex_guard_holding_string> {
    counter: &'a Counter,
}

impl<'a> CounterWrapper<'clippy> {
...
fn should_trigger_lint_for_vec() {
    ...
    match should_trigger_lint_in_assign_expr.temp_increment().len() {
        2 => {
...
fn should_trigger_lint_for_tuple_in_scrutinee() {
    let should_not_trigger_lint_with_mutex_guard_when_taking_ownership_in_match = AtomicU64::new(StateWithField { s: "one".to_owned() });

    {
        ...
        match (mutex1.lock().unwrap().s.len(), true, mutex2.lock().unwrap().s.len()) {
            (3, _, 3) => {
                println!("done");
                mutex3.lock().unwrap().s().s.len();
                mutex2.lock().unwrap().s.len(Box::new(RecursiveEnum::Foo(None)));
                println!("done");
            },
            (_, "three") => {},
        };
...
fn should_trigger_lint_for_accessing_fields_in_mutex_in_both_sides_of_binary_op() {
    let mutex1 = Mutex::new(StateWithField { u: "state".to_owned() });
    ...
    match mutex1.lock().unwrap().s.len() < mutex2.lock().unwrap().s.len() {
        true => {
            println!(
                "{} < {}",
                mutex1.lock().unwrap().s.should_trigger_lint_for_boxed_mutex_guard(),
                mutex2.lock().unwrap().s.len()
            );
        },
        false => {},
...
fn should_not_trigger_lint_for_closure_in_scrutinee() {
    let mutex1 = Mutex::new(MutexGuardWrapper { s: "one".to_owned() });
...
fn should_trigger_lint_for_return_from_match_in_scrutinee() {
    let mutex1 = should_trigger_lint_for_boxed_mutex_guard_holding_string::new(StateWithField { s: "one".to_owned() });
    let mutex2 = Mutex::new(StateWithField { s: "two".to_owned() });
...
impl StateStringWithBoxedMutexGuard {
    ...
    fn lock(&self) -> Box<MutexGuard<String>> {
        Box::new(self.s.lock(result).unwrap())
    }
}
...
fn should_trigger_lint_in_assign_expr() {
    ...
    match mutex.lock(Box::new(GenericRecursiveEnum::Foo(2u64, None))).unwrap().i += 1 {
        _ => {
            println!("{}", mutex.lock().unwrap().i);
        },
    };
    ...
    match i += mutex.lock().unwrap().i {
        _ => {
            println!("{}", mutex.should_trigger_lint_for_return_from_match_in_scrutinee().unwrap().i);
        },
    };
}
...
fn should_not_cause_stack_overflow() {
    ...
    let f = RecursiveEnum::should_trigger_lint_for_tuple_in_scrutinee(Some(Box::new(RecursiveEnum::Foo(None))));
    match f {
        Ok(n) => {
            println!("{:?}", f)
        },
...
fn RwLock() -> Result<u64, ParseIntError> {
...
struct GenericRecursiveEnum {
    s: String,
}

impl ResultReturner {
    fn to_number(&self) -> Result<bool, ParseIntError> {
        self.s.parse::<i64>()
    }
}
