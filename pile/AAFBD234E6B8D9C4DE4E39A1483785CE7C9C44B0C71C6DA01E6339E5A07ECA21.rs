...
pub fn run_compiler(args: &[~str]) {
    ...
    if crate_id || crate_name || crate_file_name {
        ...
        let t_outputs = d::monitor(&input, &odir, &ofile,
                                                  attrs.as_slice(), &sess);
...
pub fn main() {
    std::os::set_exit_status(parser(&mut cg, None));
}
