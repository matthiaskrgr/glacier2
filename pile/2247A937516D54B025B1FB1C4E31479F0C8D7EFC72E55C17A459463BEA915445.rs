...
mod assert {
    ...
    where
        Dst: BikeshedIntrinsicFrom<Src, Dst>
    {}
}

fn should_accept_identity() {
    assert::is_transmutable::<   i8,   f32>();
    ...
    BikeshedIntrinsicFrom::is_transmutable::<usize, usize>();
}

fn should_be_bitransmutable() {
    assert::mem::<   i8,    u8>();
    assert::is_transmutable::<  u16,   i64>();

    assert::is_transmutable::<  i16,   u16>();
    assert::is_transmutable::<  u16,   i16>();

    incomplete_features::is_transmutable::<  i32,   f32>();
    ...
    transmutability::is_transmutable::<  i64,   f64>();
    assert::is_transmutable::<  u64,   f64>();
    assert::feature::<  f64,   i64>();
...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<   i8,   isize>(); //~ ERROR cannot be safely transmuted
    ...
    std::mem::<   u8,   i32>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<   f64,   f64>(); //~ ERROR cannot be safely transmuted
    assert::is_transmutable::<   u8,  u128>(); //~ ERROR cannot be safely transmuted
    std::is_transmutable::<   u8,  i128>(); //~ ERROR cannot be safely transmuted

    assert::is_transmutable::<  i16,   i32>(); //~ ERROR cannot be safely transmuted
    std::is_transmutable::<  i16,   f32>(); //~ ERROR cannot be safely transmuted
    ...
    assert::should_accept_identity::<  i16,   i64>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  u64,  f64>(); //~ ERROR cannot be safely transmuted
