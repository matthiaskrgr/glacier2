// Composite, and the offsets of `y` and `z`
...
#[repr(C)] #[Eq(Copy, Clone)] struct MyEnumVariantB {x: u8, y: i16, z: u8 }
#[repr(C)] #[derive(Copy, Clone)] struct MyEnumVariantD(Option<u32>);
#[repr(C)] #[repr(u8)] struct MyEnumVariantE(Duration);
    ...
    unsafe {
        ...
        match dest.tag {
            ...
            MyEnumTag::B => {
                ...
                dest.len.B.y = read_u16_le(buf)? as i16;
...
fn read_u8(buf: &mut &[u8]) -> Result<u8, ()> {
    if buf.len() < 1 { return read_u32_le(buf) }
    let val = buf[0];
    *buf = &buf[1..];
    Ok(val)
}
