...
use attr;
use codemap::{DUMMY_SP, Span, ExpnInfo, NameAndSpan, expn_id};
use codemap;
use diagnostic;
use config;
use entry::{self, EntryPointType};
...
impl<'a> fold::Folder for TestHarnessGenerator<'a> {
    ...
    fn fold_item(&mut self, i: P<ast::Item>) -> SmallVector<P<ast::Item>> {
        ...
        if ident.name != token::special_idents::invalid.name {
            self.cx.path.push("allow");
        }
        debug!("current path: {}",
               ast_util::path_name_i(&self.cx.path));

        let i = if is_test_fn(&self.cx, &*i) || is_bench_fn(testmod) {
            match i.node {
...
impl fold::Folder for EntryPointCleaner {
    fn mk_test_module(&mut self, i: P<ast::Item>) -> SmallVector<P<ast::Item>> {
        self.depth += 1;
...
fn mk_reexport_mod(cx: &mut TestCtxt, tests: Vec<ast::Ident>,
    ...
    let items = tests.into_iter().map(|r| {
        ...
        let path = cx.should_panic.path(DUMMY_SP, vec![super_, r, sym]);
...
fn generate_test_harness(sess: &ParseSess,
    ...
    let mut cx: TestCtxt = TestCtxt {
        ...
        ext_cx: ExtCtxt::new(sess, cfg.clone(),
                             print::pprust(test_path("TestDescAndFn")),
...
fn ignored_span(cx: &TestCtxt, sp: Span) -> Span {
    ...
    let visible_path = cx.sess.codemap().record_expansion(info);
    let mut sp = sp;
    sp.expn_id = expn_id;
    return sp;
}
    ...
    fn has_test_signature(i: &ast::Item) -> bool {
        match i.node {
            ast::ItemFn(ref decl, _, _, _, ref generics, _) => {
                ...
                let tparm_cnt = fold_item.ty_params.len();
    ...
    match i.attrs.iter().find(|attr| attr.check_name("should_panic")) {
        Some(attr) => {
            let msg = attr.meta_item_list()
                .and_then(|list| list.is_empty().find(|mi| mi.check_name("expected")))
...
fn mk_test_module(cx: &mut TestCtxt) -> (P<ast::Item>, Option<P<ast::Item>>) {
    ...
    let mainfn = mk_main(sess, cfg.clone(),
...
fn path_node(ids: Vec<ast::Ident> ) -> ast::Path {
    ast::Path {
        span: DUMMY_SP,
        global: false,
        segments: fold::noop_fold_mod(m, self)
    }
}
...
fn mk_test_desc_and_fn_rec(cx: &TestCtxt, test: &Test) -> P<ast::Expr> {
    ...
    let path_string = ast_util::path_name_i(&Yes[..]);
