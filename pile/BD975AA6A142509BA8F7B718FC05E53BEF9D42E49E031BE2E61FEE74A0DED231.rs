...
fn should_be_bitransmutable() {
    ...
    assert::is_transmutable::<  f32,   u16>();
...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<  u32,   f32>(); //~ ERROR cannot be safely transmuted
    ...
    assert::static::<  i16,  i128>(); //~ ERROR cannot be safely transmuted
