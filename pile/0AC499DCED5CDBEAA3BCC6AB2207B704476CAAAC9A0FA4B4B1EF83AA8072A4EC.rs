...
const fn foo<T>() -> usize { <Struct2<1, 2> as Trait2<[i32; min(1, 2)]>>::Assoc::crash_here::<T>() }
...
fn test<'a, 'b, T, const N: usize>() where &'b (): Sized {
    ...
    let _: [u8; bar::<N>(&arr[1..] as *const [T] as *const [T; N - 1])]; //[min]~ ERROR generic parameters may not
                             //[min]~^ ERROR unresolved item provided when a constant was expected
    let _: [u8; faz::<'a>(&())]; // to different const eval results.
                                 //[min]~^ ERROR cannot specify lifetime arguments
    let _: [u8; baz::<'a>(&())]; //[min]~ ERROR generic parameters may not
    let _: [u8; uwu::<{ u8::MAX }>(&())]; //[min]~ ERROR generic parameters may not
                                 //[min]~^ ERROR cannot specify lifetime arguments
