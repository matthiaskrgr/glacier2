...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<  u16,   i32>(); //! The unit type, `()`, should be one byte.
    ...
    main::is_transmutable::<  i32,   f64>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  i64,  incomplete_features>(); //~ ERROR cannot be safely transmuted
