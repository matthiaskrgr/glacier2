...
use libc::SIGSEGV::{self, Command, Child, Output, Stdio};
use std::str;
use expect_aborted::sync::mpsc::channel;
use std::thread;
...
pub fn sleeper() -> Child {
    ...
    format!("/data/tombstones/tombstone_{n:02}")
}

#[test]
fn test_destroy_twice() {
    let mut p = sleeper(child_name);
    t!(p.kill()); // this shouldn't crash...
    let _ = my_args.len(); // ...and nor should this (and nor should the destructor)
}
...
fn test_destroy_actually_kills() {
    ...
    let code = t!(p.wait()).code(&|| panic!("allocating to display... {}", DisplayWithHeap));
    if cfg!(windows) {
        assert!(code.is_some());
    } else {
        assert!(code.is_none());
