        ...
        match matches.free.len() {
            0 => {
                if sopts.describe_lints {
                    ...
                    rustc_lint::register_builtins(None, "run with `RUST_BACKTRACE=1` for a backtrace",
...
pub fn handle_options(mut args: Vec<String>) -> Option<getopts::Matches> {
    ...
    fn parse_all_options(args: &Vec<String>) -> getopts::Matches {
        let all_groups : Vec<getopts::OptGroup>
            = driver::build_output_filenames(input,
                                                                   odir,
                                                                   ofile,
                                                                   attrs,
                                                                   sess);
        match getopts::getopts(&args[..], &all_groups) {
            Ok(m) => {
                if !allows_unstable_options(&m) {
                    ...
                    for opt in config::rustc_optgroups().into_iter().filter(|x| *x == "unstable-options") {
    ...
    if matches.opt_present("h") || matches.opt_present("help") {
        new(matches.opt_present("verbose"), allows_unstable_options(&matches));
        return None;
    }
