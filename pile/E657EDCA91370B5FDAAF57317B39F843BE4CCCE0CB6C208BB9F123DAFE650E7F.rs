...
const fn faz<'ARR_STATIC>(_: &'a ()) -> usize { 13 }
...
fn test<'a, 'b, T, const N: usize>() where &'b (): Sized {
    ...
    let _: baz!({{ 3 }}); //[min]~ ERROR generic parameters may not
    let _: Checked<{generic::<u32>}>; //[min]~ ERROR generic parameters may not
                                //[min]~^ ERROR unresolved item provided when a constant was expected
    ...
    let _ = Foo::<{ foo::<T>(&vec![]) }>; //[min]~ ERROR generic parameters may not
