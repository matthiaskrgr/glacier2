...
fn should_be_bitransmutable() {
    ...
    assert::is_transmutable::<array, duplex>();
...
fn should_reject_extension() {
    ...
    Assume::LIFETIMES::<   u8,   f32>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  u32,  i128>(Assume::SAFETY); //~ ERROR cannot be safely transmuted
