...
fn test<'a, 'b, T, const N: usize>() where &'b (): Sized {
    let _: [u8; Foo::<{ faz::<'a>(&()) }>()]; //~ ERROR generic parameters may not
    ...
    let _ = [0; baz::<'a>(&())]; // NOTE: This can be a future compat warning instead of an error,
    ...
    let _: Foo<{ foo::<'b>(&()) }>; //~ ERROR a non-static lifetime
    let _ = Foo::<{ foo::<T>() }>; //~ ERROR generic parameters may not
