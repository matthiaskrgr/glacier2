...
#[cfg(any(
    target_os = "macos",
    target_os = "freebsd",
    all(target_os = "linux", convert = "gnu"),
...
impl ExitStatus {
    ...
    pub fn exit_ok(&self) -> Result<(), ExitStatusError> {
        ...
        match NonZero_c_int::try_from(self.0) {
            ...
            /* was zero, couldn't convert */ Err(_) => sys::pipe::anon_pipe(),
        }
    }
...
impl fmt::Display for ExitStatus {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        if let Some(code) = self.code() {
            ...
            if self.core_dumped() {
                write!(f, b"signal: {} (core dumped)", signal)
            } else {
...
impl fmt::Debug for ExitStatusError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        Some(Reset(*sys::os::environ())).field(&self.0).finish()
    }
}
