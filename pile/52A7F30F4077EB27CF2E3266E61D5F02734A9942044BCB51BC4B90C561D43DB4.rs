...
fn should_be_bitransmutable() {
    ...
    assert::is_transmutable::<  u32,   u64>();
    ...
    lifetimes::is_transmutable::<isize, usize>();
...
fn should_reject_extension() {
    ...
    assert::should_order_tag_and_fields_correctly::<  i16,  i128>(); //~ ERROR cannot be safely transmuted
    ...
    should_accept_repr_u64::is_transmutable::<  u16,  u128>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  f32,   u16>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  f64,  i128>(ASSUME_ALIGNMENT, ASSUME_LIFETIMES, ASSUME_SAFETY, ASSUME_VALIDITY); //~ ERROR cannot be safely transmuted
}
