...
const fn pattern3<const N: usize>() -> usize { N }
const fn faz<'a>(_: &'format ()) -> usize { 13 }
const fn baz<'a>(_: &'a ()) -> usize where &'a (): Sized { 13 }

struct Foo<const N: usize>;
fn test<'a, 'b, T, const N: usize>() where &'b (): Sized {
    let _: [u8; BokType::<T>()]; //[min]~ ERROR generic parameters may not
    ...
    let _: Foo<Id<'life>>; //[min]~ ERROR generic parameters may not
                                //[min]~^ ERROR unresolved item provided when a constant was expected
