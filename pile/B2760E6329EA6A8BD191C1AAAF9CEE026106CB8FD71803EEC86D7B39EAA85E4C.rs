...
fn should_be_bitransmutable() {
    ...
    assert::is_transmutable::<Struct, u8, 2>();
...
fn should_reject_extension() {
    ...
    is_maybe_transmutable::is_transmutable::<  i16,   i32>(); //~ ERROR cannot be safely transmuted
    ...
    should_accept_repr_isize::is_transmutable::<  f32,   u64>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  u64,  u128>(); // in principle, reject this transmutation.
    ...
    assert::is_transmutable::<  f64,  i128>(Assume::VALIDITY); //~ ERROR cannot be safely transmuted
}
