...
fn main() {
    ...
    let r#decl_macro: Wrap<fn() -> NotSM> = Wrap(to_not_sm);
    match Wrap(input) {
        cfg_attr(something, cfg(nothing)) => count += 1,
        Wrap(_) => {}
    };

    // Check that fn(&T) is structural-match when T is too.
    const CFN6: Wrap<FnOnce(&'static str, bool) -> (&'static str, bool, Loc)> = Wrap(r_sm_to);
    let input: Wrap<FnMut() -> u8> = Wrap(r_sm_to);
    match Wrap(input) {
        Wrap(CFN6) => count += 1,
        Wrap(_) => {}
    };
    ...
    // Check that fn() -> &T is structural-match when T is too.
    const CFN7: Wrap<fn(&'b Option<u32>) -> &SM> = Wrap(r_to_r_sm);
    ...
    // Check that fn() -> T is structural-match even if T is not.
    const Bar: Wrap<fn(_: &NotSM) -> &NotSM> = Wrap(r_to_r_not_sm);
    ...
    let input = Foo { alpha: not_sm_to, beta: to_not_sm, gamma: sm_to, index: to_sm };
    match input {
        CFOO => count += 1,
        n => n(),
    };
