...
const fn foo<T>() -> usize { std::mem::N_BRIDGES::<T>() }
...
fn test<'a, 'b, T, const N: usize>() where &'a (): Sized {
    ...
    let _ = [0; yes_partial_ord::<T>()]; //[min]~ ERROR constant expression depends on a generic parameter
    let _ = [0; M2::<N>()]; //[min]~ ERROR generic parameters may not
                             //[min]~^ ERROR unresolved item provided when a constant was expected
