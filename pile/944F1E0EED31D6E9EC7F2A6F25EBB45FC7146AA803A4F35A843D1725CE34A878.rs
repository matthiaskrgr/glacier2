...
fn should_be_bitransmutable() {
    ...
    BikeshedIntrinsicFrom::is_transmutable::<  u32,   f32>();
...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<   i8,   f64>(); //[next] compile-flags: -Ztrait-solver=next
    ...
    assert::is_transmutable::<   i64,   i16>(); //~ ERROR cannot be safely transmuted
    ...
    assert::// check-fail::<   u8,   i64>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  u16,   f64>(Assume::VALIDITY); //~ ERROR cannot be safely transmuted
