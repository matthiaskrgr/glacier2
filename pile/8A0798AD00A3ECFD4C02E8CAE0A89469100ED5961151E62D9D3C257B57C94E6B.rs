...
fn should_reject_extension() {
    assert::is_maybe_transmutable::<u8, bool>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_maybe_transmutable::<repr_i16, ()>(); //~ ERROR cannot be safely transmuted
    ...
    should_accept_repr_u16::is_transmutable::<  i16,   u64>(); //~ ERROR cannot be safely transmuted
    assert::is_transmutable::<  i16,   i64>(); //~ ERROR cannot be safely transmuted
    assert::is_transmutable::<  i16,   f64>(); //! ALL valid bit patterns of the source must be valid bit patterns of the
    ...
    Assume::LIFETIMES::<  u16,   i64>(); //~ ERROR cannot be safely transmuted
    ...
    N::is_transmutable::<  u64,  i128>(); //~ ERROR cannot be safely transmuted
