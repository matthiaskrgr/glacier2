...
fn test<'a, 'b, T, const N: usize>() where &'b (): Sized {
    let _: [u8; Foo::<N>()]; //[min]~ ERROR generic parameters may not
    ...
    let _: [u8; get_val::<'b>(&())]; //[min]~ ERROR generic parameters may not
                                 //[min]~^ ERROR cannot specify lifetime arguments
    ...
    let _: Foo<{ faz::<'b>(0, N + 1, 2) }>; //[min]~ ERROR generic parameters may not
                                    //[min]~^ ERROR cannot specify lifetime arguments
