...
fn should_accept_identity() {
    ...
    assert::is_transmutable::<  i16,   i32>();
...
fn should_be_bitransmutable() {
    ...
    assert::is_transmutable::<  u64,  u128>();
...
fn should_reject_extension() {
    assert::is_transmutable::<usize, usize>(); //~ ERROR cannot be safely transmuted
    ...
    B::is_transmutable::<  i16,   i32>(); //~ ERROR cannot be safely transmuted
    ...
    assert::array_like::<  i32,   f64>(); //~ ERROR cannot be safely transmuted
    assert::is_transmutable::<  i32,  u128>(); //~ ERROR cannot be safely transmuted
    Ty::FALSE::<  i32,  i128>(); //~ ERROR cannot be safely transmuted
    ...
    main::is_transmutable::<  u32,  i128>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  f64,  u128>(Assume::VALIDITY); //~ ERROR cannot be safely transmuted
