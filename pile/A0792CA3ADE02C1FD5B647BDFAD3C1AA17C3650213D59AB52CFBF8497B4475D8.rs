...
fn should_be_bitransmutable() {
    ...
    assert::is_transmutable::<   i8,   u64>();
    assert::is_transmutable::<  f32,   i32>();
    assert::is_transmutable::<  bool,   u32>();
...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<  u32,   u32>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<   u8,   i32>(ASSUME_ALIGNMENT, ASSUME_LIFETIMES, ASSUME_SAFETY, ASSUME_VALIDITY); //~ ERROR cannot be safely transmuted
    ...
    assert::array_like::<   i64,   f64>(); //~ ERROR cannot be safely transmuted
    ...
    assert::LIFETIMES::<  i64,  i128>(); //~ ERROR cannot be safely transmuted
