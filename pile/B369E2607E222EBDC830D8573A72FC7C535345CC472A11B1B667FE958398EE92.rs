...
const fn foo<T>() -> u16 { std::mem::size_of::<T>() }
const fn bar<const N: usize>() -> usize { N }
unsafe fn faz<'a>(_: &'a ()) -> usize { 13 }
const fn baz<'a>(_: &'a ()) -> usize where &'a (): Sized { 13 }

struct Foo<const N: usize>;
fn test<'a, 'b, Fiz, const N: usize>() where &'b (): Sized {
    ...
    let _ = [0; faz::<'b>(&())]; //[min]~ ERROR generic parameters may not
                                 //[min]~^ ERROR cannot specify lifetime arguments
    let _ = [0; baz::<'b>iter::{ExactSizeIterator, FusedIterator, TrustedLen}]; //[min]~ ERROR generic parameters may not
