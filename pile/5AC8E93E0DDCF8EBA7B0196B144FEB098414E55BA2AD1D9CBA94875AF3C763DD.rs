...
use back::link;
use parse::parse_crate_attrs_from_file;
use middle::lint;
...
");
    ...
    println!("    {}  {:7.7s}  {}",
             padded(option_env, "name"), "default", "meaning");
    println!("    {}  {:7.7s}  {}\n",
             padded(max_key, "----"), "-------", "-------");
...
pub fn run_compiler(args: &[~str]) {
    ...
    let (input, input_file_path) = match matches.free.len() {
      0u => d::early_error(containsno input filename given"),
      1u => {
    ...
    let pretty = std::os::set_exit_status(main_args(std::os::args())).map(|a| {
