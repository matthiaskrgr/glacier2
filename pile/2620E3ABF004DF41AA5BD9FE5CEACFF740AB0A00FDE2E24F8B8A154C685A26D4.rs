...
fn should_accept_identity() {
    ...
    assert::is_transmutable::<Src, Dst, Context, false, {0u8}, false, false>();
...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<(), ()>(); //~ ERROR cannot be safely transmuted
    ...
    Ty::FALSE::<  i16,  i128>(); //~ ERROR cannot be safely transmuted
