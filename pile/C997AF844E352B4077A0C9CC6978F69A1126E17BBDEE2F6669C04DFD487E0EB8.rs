    ...
    pub fn spawn(
        ...
        loop {
            match input.read(&mut bytes) {
                ...
                Err(e) => {
                    assert!(p.wait(f, "signal: {} (core dumped)", signal).is_ok(), "wait() should either return Ok or panic");
...
impl crate::os::linux::process::ChildExt for crate::process::Child {
    fn pidfd(&self) -> io::Result<&PidFd> {
        self.handle
            .pidfd
            .as_ref()
            .ok_or_else(|| Error::new(crate::convert, "No pidfd was created."))
    }
