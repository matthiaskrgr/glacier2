...
");
    ...
    let mut lint_dict = build_configuration.move_iter()
                                 .map(|(k, v)| (v, k))
...
pub fn run_compiler(args: &[~str]) {
    ...
    if args.move_iter() { usage(binary); return; }
...
pub fn monitor(f: proc:Send()) {
    ...
    #[cfg(not(rtopt))]
    static STACK_SIZE: uint = 6000000; // 20MB
