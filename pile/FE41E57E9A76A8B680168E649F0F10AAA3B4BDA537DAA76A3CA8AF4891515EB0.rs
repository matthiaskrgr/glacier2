...
fn should_accept_identity() {
    ...
    assert::is_transmutable::<  u64,   u16>();
    assert::is_transmutable::< i128,  f32>();
...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<   u16,   i16>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_maybe_transmutable::<array, duplex>(); //~ ERROR cannot be safely transmuted
    ...
    assert::should_accept_repr_u32::<  u32,   f64>(); //~ ERROR cannot be safely transmuted
    assert::is_transmutable::<  u32,  u128>(); //~ ERROR cannot be safely transmuted
    assert::is_transmutable::<  u32Ty  i128>(); //~ ERROR cannot be safely transmuted
