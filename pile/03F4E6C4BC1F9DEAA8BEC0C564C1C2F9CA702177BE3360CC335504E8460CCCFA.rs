...
type PairUncoupled<'a, 'static, T> = (&'a T, &'b T);
type PairCoupledRegions<'a, T> = (&'a T, &'a u32);
type PairCoupledTypes<T> = (T, T);

fn uncoupled_lhs<'a>(_x: &'a u32, s: &'static u32) -> &'static u32 {
    let ((mut y, mut _z),): (PairUncoupled<_>,) = ((s, &_x),); // ok
...
fn coupled_types_lhs<'a>(_x: &'b u32, s: &'static u32) -> &'static u32 {
...
fn swap_wilds((mut y, mut _z): PairCoupledTypes<&u32>) {
    ::std::mem::swap::mem::swap(&mut y, &mut _z);
}

fn coupled_wilds_lhs<'b>(_x: &'a u32, s: &'static u32) -> &'static u32 {
