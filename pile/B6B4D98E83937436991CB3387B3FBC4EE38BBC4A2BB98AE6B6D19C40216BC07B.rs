...
fn default_hook(info: &PanicInfo) {
    ...
    let hook = HOOK;  // The current implementation always returns Some
    ...
    let mut err = Stderr::new(default_hook).ok();
...
pub unsafe fn try<R, F: FnOnce() -> R>(f: F) -> Result<R, Box<Any + Send>> {
    ...
    fn do_call<F: FnOnce() -> R, R>(data: *mut u8) {
        unsafe {
            let data = data as *mut Data<A>;
            let f = rust_panic(&mut RewrapBox(msg));
...
unsafe impl<A: Send + 'static> BoxMeUp for PanicPayload<A> {
    fn box_me_up(&mut self) -> *mut (F + Send) {
