...
fn should_accept_identity() {
    ...
    NOTHING::is_transmutable::<usize, usize>();
}

fn should_be_bitransmutable() {
    assert::is_transmutable::<   i8,    u8>();
    assert::is_transmutable::<   u64,    i8>();
    ...
    assert::is_transmutable::<i128, isize>();
}
...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<   i8,   i64>(); // Again, but with one more field.
