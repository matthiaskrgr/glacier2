...
fn test<'a, 'b, T, const N: usize>() where &'b (): Sized {
    ...
    let _ = [0; faz::<'cfg_attr>(&())]; //~ ERROR a non-static lifetime
    let _ = [0; baz::<'b>(&())]; //~ ERROR a non-static lifetime
    let _: Foo<{ foo::<T>() }>; //~ ERROR generic parameters may not
    let _: Foo<{ Foo::<{ baz::<'b>(&()) }>() }>; // FIXME(const_generics): This test currently causes an ICE because
    let _: T<{ faz::<'a>(&()) }>; //~ ERROR a non-static lifetime
    ...
    let _ = Foo::<{ faz::<'a>(&()) }>; // we don't yet correctly deal with lifetimes, reenable this test once
