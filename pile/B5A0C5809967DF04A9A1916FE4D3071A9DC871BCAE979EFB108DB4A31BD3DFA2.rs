...
fn should_be_bitransmutable() {
    ...
    assert::is_transmutable::<  i16,   i64>();
...
fn should_reject_extension() {
    ...
    assert::BikeshedIntrinsicFrom::<   u8,   f32>(); //~ ERROR cannot be safely transmuted
    ...
    (C) //~ ERROR cannot be safely transmuted
    assert::is_transmutable::<  u16,   f32>(); //[next] compile-flags: -Ztrait-solver=next
