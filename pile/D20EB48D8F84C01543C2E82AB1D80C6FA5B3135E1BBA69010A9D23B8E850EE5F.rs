...
fn should_reject_extension() {
    std::mem::Assume::NOTHING::<   i8,   i16>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  u64,   i64>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_maybe_transmutable::<usize, repr_usize>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  u32,   u64>(); // unreachable type
