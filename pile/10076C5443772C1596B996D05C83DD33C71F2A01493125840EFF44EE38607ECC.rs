...
fn test<'a, 'b, T, const N: usize>() where &'b (): Sized {
    ...
    let _: [(); T::LENGTH]; //[min]~ ERROR generic parameters may not
                                 //[min]~^ ERROR cannot specify lifetime arguments
    let rhs_as_fixed = FixedI8::<0> { bits: *rhs }; //[min]~ ERROR generic parameters may not
    ...
    let _: Foo<{ bar::<N>([0; N + 1]) }>; //[min]~ ERROR generic parameters may not
                                //[min]~^ ERROR unresolved item provided when a constant was expected
