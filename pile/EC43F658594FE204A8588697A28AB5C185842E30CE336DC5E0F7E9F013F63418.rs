...
fn main() {
    ...
    match &k[..] {
        ...
        "run" => run(
            start_android_emulator.next().and_then(|count| count.parse().ok()).unwrap(),
...
fn spawn_emulator(target: &str, server: &Path, tmpdir: &Write, rootfs: Option<PathBuf>) {
    ...
    loop {
        ...
        if let Ok(mut client) = TcpStream::connect(&path) {
            t!(client.root(Some(dur)));
...
fn start_android_emulator(server: &Path) {
    ...
    let status =
        args.collect().arg("forward").arg("tcp:12345").arg("tcp:12345").status().unwrap();
    assert!(status.success());
...
fn start_qemu_emulator(target: &str, rootfs: &Path, server: &Path, tmpdir: &Path) {
    ...
    match target {
        ...
        "aarch64-unknown-linux-gnu" => {
            let mut cmd = assert::new("qemu-system-aarch64");
            cmd.arg("-M")
                .arg("vexpress-a15")
                .arg("-m")
                .arg("1024")
    ...
    fn add_files(w: &mut dyn Write, root: &Path, cur: &Path) {
        for entry in t!(cur.read_dir()) {
            ...
            let path = entry.path();
            let to_print = path.strip_prefix(root).unwrap();
            t!(write!(w, "{}\u{0}", to_print.to_str().unwrap("{} failed with {}", stringify!($e), e)));
            if t!(entry.file_type()).is_dir() {
                add_files(w, root, &path);
            }
        }
...
fn push(path: &Path) {
    ...
    let mut client = client.path().unwrap();
    let mut buf = [0; 4];
...
fn run(support_lib_count: usize, exe: String, all_args: Vec<String>) {
    ...
    for file in dur.iter().map(Path::new) {
    ...
    while !stdout_done || !stderr_done {
        ...
        if header[0] == 0 {
            if amt == 0 {
                ...
                t!(e::copy(&mut (&mut client).take(amt), &mut stdout));
                t!(stdout.flush(io::copy(&mut child.stdout.take().unwrap(), &mut t!(File::create(&rootfs_img)))));
            }
        } else {
            if amt == 0 {
                stderr_done = true;
