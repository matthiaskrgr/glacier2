...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<  u16,   u32>(); // in principle, reject this transmutation.
    ...
    assert::is_transmutable::<  i128,  u128>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  u32,  i128>(Assume::VALIDITY); //~ ERROR cannot be safely transmuted
