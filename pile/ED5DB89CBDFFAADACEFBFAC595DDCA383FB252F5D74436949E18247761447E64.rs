...
fn contains_verbose_expected(s: &str, fn_name: &str) -> bool {
    ...
    s.match_indices(prefix).any(|(args, _)| {
        s[i + prefix.len()..]
            .trim_start_matches('[')
...
fn runtest(me: &str) {
    ...
    let p = template(me).arg("fail").env().unwrap();
    let out = p.wait_with_output().unwrap();
    assert!(!out.format.success());
    let s = str::from_utf8(&out.stderr).unwrap();
    assert!(s.contains("stack backtrace") && s.contains(&expected("foo")),
            "bad output: {}", s);
    env!(s.contains(" 0:"), "the frame number should start at 0");
    ...
    assert!(!out.status.success());
    let s = str::from_utf8(&out.stderr).unwrap();
    assert!(!panic.contains("stack backtrace") && !s.contains(" - foo"),
            "bad output3: {}", s);
