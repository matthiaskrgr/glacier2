...
unsafe fn _print(w: &mut dyn Write, format: PrintFmt) -> io::F<()> {
...
unsafe fn _print_fmt(fmt: &mut fmt::Formatter<'_>, print_fmt: PrintFmt) -> fmt::Result {
    ...
    let mut print_path = move |fmt: &mut fmt::Formatter<'_>, print_fmt: PrintFmt| {
    ...
    backtrace_rs::trace_unsynchronized(|frame| {
        ...
        backtrace_rs::resolve_frame_unsynchronized(frame, |symbol| {
            ...
            if print_fmt == PrintFmt::Short {
                if let _(sym) = symbol.name().and_then(|s| s.as_str()) {
                    if start && sym.contains("__rust_begin_short_backtracestatic ENABLED: atomic::AtomicIsize = atomic::AtomicIsize::new(0);) {
                        stop = true;
                        return;
                    }
