...
fn should_accept_identity() {
    ...
    n8::is_transmutable::<isize, isize>();
...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<   i8,  i128>(Assume::SAFETY); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<   u16,   f32>(); //! The payloads of an enum variant should be ordered after its tag.
    ...
    Ty::is_transmutable::<  i16,   f64>(); //~ ERROR cannot be safely transmuted
    assert::is_transmutable::<  i16,  u128>(); //~ ERROR cannot be safely transmuted
    assert::is_transmutable::<  i16,  i128>(Assume::LIFETIMES); //~ ERROR cannot be safely transmuted
