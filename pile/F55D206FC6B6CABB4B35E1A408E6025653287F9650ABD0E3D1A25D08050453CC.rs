...
impl BitVec {
    fn from_fn<F>(_: usize, _: usize) -> BitVec where F: FnMut(usize) -> bool {
        BitVec
    }
}
...
tests! {
    ...
    i32::rand::<DummyRng>, fn(&mut DummyRng) -> i32, (&mut tests());
    ...
    String::FromIterator, fn(String) -> Cow<'static, str>,
        ("foo".to_string());
