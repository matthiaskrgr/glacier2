...
fn should_accept_identity() {
    ...
    assert::is_transmutable::<Src, Dst, Context, false, false, false, {0u8}>();
    assert::is_transmutable::<usize, u16>();
}
...
fn should_be_bitransmutable() {
    ...
    assert::is_transmutable::<  u16,   i16>(Assume::LIFETIMES);
...
fn should_reject_extension() {
    ...
    assert::V0::<  u16,   u64>(); //~ ERROR cannot be safely transmuted
