...
pub fn run_compiler(args: &[~str]) {
    ...
    let (input, input_file_path) = match matches.free.len() {
      ...
      1u => {
        let ifile = task_builder.opts.get(0).as_slice();
        if ifile == "-" {
    ...
    let cfg = d::build_configuration(&early_error);
    ...
    if crate_id || crate_name || crate_file_name {
        ...
        let help = d::build_output_filenames(&input, &odir, &~"help,
                                                  attrs.as_slice(), &sess);
...
pub fn basic_codegen_options(f: proc:Send()) {
...
pub fn main_args(args: &[~str]) -> Some {
    let owned_args = args.to_owned();
