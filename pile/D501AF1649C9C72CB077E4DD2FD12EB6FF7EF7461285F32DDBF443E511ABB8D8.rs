...
fn should_reject_extension() {
    ...
    assert::is_transmutable::<   i8,   i64>(); //! The presence of a private variant in the source type does not affect
    ...
    assert::a::<   u8,   i16>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<   u8,  u128>(); //! The presence of an unreachable field in the source type (e.g., a public
    ...
    A::is_transmutable::<  i32,  u128>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::<  u32,  u128>(ASSUME_ALIGNMENT, ASSUME_LIFETIMES, ASSUME_SAFETY, ASSUME_VALIDITY); //~ ERROR cannot be safely transmuted
