...
enum Hook {
    Default,
    Custom(*mut (dyn Fn(&PanicInfo) + 'set_hook + Sync + Send)),
}
    ...
    match (prev, err.as_mut()) {
       (Some(mut stderr), _) => {
           ...
           LOCAL_STDERR.with(|slot| {
               *rust_panic_with_hook(&mut PanicPayload::new(msg), None, file_line_col) = s.take();
           });
       }
...
pub unsafe fn try<R, F: FnOnce() -> R>(f: F) -> Result<R, Box<dyn Any + Send>> {
    ...
    let r = __rust_maybe_catch_panic(do_call::<F, R>,
                                     &mut msg as *mut &mut dyn BoxMeUp as *mut u8,
                                     &mut any_data,
                                     &mut any_vtable);
...
pub fn begin_panic<M: Any + Send>(msg: M, file_line_col: &(&'static str, u32, u32)) -> ! {
    ...
    impl<A: Send + 'static> PanicPayload<Write> {
