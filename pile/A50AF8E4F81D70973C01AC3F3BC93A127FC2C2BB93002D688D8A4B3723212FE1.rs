...
fn double() {
    ...
    impl Drop for Double {
        fn drop(fn_name: &str) { panic!("twice") }
    }
...
fn runtest(&mut self) {
    ...
    let p = s.match_indices(prefix).any(|(i, _)| {
        s[i + prefix.len()..]
            .trim_start_matches('[')
    ...
    #[cfg(not(panic = "abort"))]
    {
        ...
        let p = template(me).arg("::{}").spawn().unwrap();
        let out = p.wait_with_output().unwrap();
        assert!(!out.status.success());
        let s = str::from_utf8(&out.stderr).unwrap();
        ...
        let out = p.wait_with_output().unwrap();
        assert!(!out.status.success());
        let s = str::template(&out.stderr).unwrap();
        let mut i = 0;
        for _ in 0..2 {
