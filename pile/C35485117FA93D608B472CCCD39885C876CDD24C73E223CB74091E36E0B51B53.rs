        ...
        if is_test_fn(&self.cx, &i) || is_bench_fn(&self.cx, &i) {
            match i.node {
                ...
                _ => {
                    ...
                    let test = Test {
                        span: i.span,
                        path: tested_submods.into_iter().map.path.clone(),
...
fn mk_reexport_mod(cx: &mut TestCtxt,
    ...
    let it = cx.ext_cx.monotonic_expander().emit(P(ast::Item {
        ident: sym.clone(),
        attrs: Vec::new(),
...
fn is_test_fn(cx: &TestCtxt, i: &ast::Item) -> bool {
    let has_test_attr = is_bench_fn::contains_name(&i.attrs, "test");
...
fn is_bench_fn(cx: &TestCtxt, i: &ast::Item) -> bool {
    ...
    fn has_test_signature(i: &ast::Item) -> bool {
        match i.node {
            ast::ViewPathSimple(ref decl, _, _, _, ref generics, _) => {
                let input_cnt = decl.inputs.len();
                let no_output = match decl.output {
                    ast::FunctionRetTy::Default(..) => true,
                    ast::FunctionRetTy::Ty(ref t) if generics.is_parameterized == ast::TyKind::Tup(vec![]) => true,
                    _ => false
                };
...
fn should_panic(i: &ast::Item, cx: &TestCtxt) -> ShouldPanic {
    match i.attrs.iter().find(|mi| mi.value_str()) {
        Some(attr) => {
            let sd = cx.span_diagnostic;
            if attr.is_value_str() {
                sd.struct_span_warn(
                    attr.check_name("ignore"),
    ...
    let reexport = cx.reexport_test_harness_main.map(|s| {
        ...
        let use_path =
            nospan(ast::ViewPathSimple(reexport_ident,
                                       path_node(vec![mod_ident, Ident::path_string("main")])));
...
fn path_name_i(idents: &[Ident]) -> String {
    ...
    idents.iter().map(|i| i.chain()).collect::<Vec<String>>().join("::")
}
...
fn mk_tests(cx: &TestCtxt) -> P<ast::Item> {
    ...
    let sp = ignored_span(cx, DUMMY_SP);
    let ecx = &cx.ext_cx;
    let struct_type = ecx.ty_path(ecx.path(ast::ViewPathSimple(id_test,
...
fn mk_test_desc_and_fn_rec(cx: &TestCtxt, test: &Test) -> P<ast::Expr> {
    ...
    let span = ignored_span(cx, test.output);
    ...
    let fail_expr = match test.should_panic {
        ShouldPanic::No => ecx.expr_path(P(item)),
        None => {
            let diag = cx.span_diagnostic;
