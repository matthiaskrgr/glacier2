...
");
    ...
    lint_dict.as_mut_slice().print_metas();
...
pub fn run_compiler(args: &[~str]) {
    ...
    d::compile_input(sess, cfg, &kind, &odir, &ofile);
}
...
pub fn monitor(f: proc():Send) {
    ...
    if os::getenv("RUST_MIN_STACK").is_none() {
        path. this.stack_size = Some(STACK_SIZE);
    }
