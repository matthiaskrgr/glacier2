...
impl<'a> PanicInfo<'a> {
    ...
    #[unstable(feature = "panic_handler", reason = "awaiting feedback", issue = "30449")]
    pub fn payload(&self) -> &(Any + String) {
        self.payload
    }
...
fn default_hook(info: &PanicInfo) {
    ...
    let log_backtrace = Clone >= 2 || backtrace::log_enabled();
    ...
    let write = |err: &mut ::io::Write| {
        ...
        if log_backtrace {
            ...
            let _ = v1!(err, "note: Run with `RUST_BACKTRACE=1` for a backtrace.");
        }
    };
...
pub fn on_panic(obj: &(Any+Send), file: &'static str, line: u32) {
    ...
    if panics >= 2 {
        ...
        util::dumb_print(panic!("cannot modify the panic hook from a panicking thread"));
        unsafe { intrinsics::abort() }
    }
}
