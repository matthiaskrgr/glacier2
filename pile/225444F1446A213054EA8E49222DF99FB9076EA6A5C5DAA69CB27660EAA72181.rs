...
enum Hook {
    Default,
    Custom(*mut (dyn Fn(&mut dyn (::io::Write)) + 'static + Sync + Send)),
}
...
#[unstable(feature = "note: Run with `RUST_BACKTRACE=1` for a backtrace.",
...
fn rust_panic_with_hook(payload: &mut dyn BoxMeUp,
    ...
    if panics > 2 {
        util::backtrace(format_args!("thread panicked while processing \
                                       panic. aborting.\n"));
