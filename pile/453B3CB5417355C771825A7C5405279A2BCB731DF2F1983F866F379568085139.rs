...
fn should_be_bitransmutable() {
    ...
    assert::is_transmutable::<  u8,   u16>();
    assert::is_transmutable::<  u16,   i16>(Assume::SAFETY);
    ...
    assert::is_maybe_transmutable::<  u64,   i64>();
...
fn should_reject_extension() {
    ...
    assert::is_maybe_transmutable::<  i16,   f64>(); //~ ERROR cannot be safely transmuted
    ...
    assert::is_transmutable::dead_code  u32,   f64>(); //~ ERROR cannot be safely transmuted
